...
# --- general queries ---
qry_get_maps = "select distinct MapName from GameExt where FirstSeeker like 'single2016_test7%' and not EndTime is null and not MapName like '%master29f%';"

# matrix with on each row: phase, variable, description, sql-query
qrys = [
["find", "steps",
"find - first visible step",
"""select concat_ws(',',MapPubName,AutoWalkerN,SolverTypePubName,right(FirstSeeker,10)) as Params, FirstVisibStep
from GameExt g  left join FirstVisibStep f on g.id=f.GameID
where FirstSeeker like 'single2016_test7%' and not EndTime is null and SimObsNoiseStd=0 and MapName='<MAP>' and AutoWalkerN=<AUTOW>
order by Params;"""
],
["find","steps",
"find - first close steps",
"""select concat_ws(',',MapPubName,AutoWalkerN,SolverTypePubName,right(FirstSeeker,10)) as Params, FirstClose3Step
from GameExt g  left join FirstClose3Step fc3 on g.id=fc3.GameID
where FirstSeeker like 'single2016_test7%' and not EndTime is null and SimObsNoiseStd=0 and MapName='<MAP>' and AutoWalkerN=<AUTOW>
order by Params;"""
],
["find","SeekerBeliefScore",
"find - SeekerBeliefScore",
""""select concat_ws(',',MapPubName,AutoWalkerN,SolverTypePubName,right(FirstSeeker,10)) as Params,SeekerBeliefScore
from GameExt g left join GameLine gl on g.id=gl.GameID
    left join GameUserLine gul on gl.id = gul.GameLineID 
    left join GameUser gu on gu.id=gul.GameUserID
where  FirstSeeker like 'single2016_test7%' and not EndTime is null and SimObsNoiseStd=0 and MapName='<MAP>' and AutoWalkerN=<AUTOW> and gu.IsSeeker=1
order by Params;"""
]
