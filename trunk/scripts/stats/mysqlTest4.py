...
TABLE_WIDTH = 86
TABLE = "SimJuly2013f" #"SimMay2013ss"
...
"""select concat_ws(',',SeekerType2,Reward3,HiderCat3 ), sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total
from <TABLE>
where MapSize<>'40x40' and HasTimeLimit='no'
group by SeekerType2,HiderCat3,Reward3
order by SeekerType2,Reward3,HiderCat3"""],

["tab2: off Reward3s",
"""select Reward3, sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total
from <TABLE>
where OnOffline='offline' and HasTimeLimit='no'
group by Reward3"""],

["tab2b: off Reward3s, HiderCat3",
"""select concat_ws(',',Reward3,HiderCat3), sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total
from <TABLE>
where OnOffline='offline' and HasTimeLimit='no'
group by Reward3,HiderCat3"""],
...
,
["tab9: on/off seek type, hider type + mapid",
"""select concat_ws(',',MapID,HiderCat3,SeekerType2,Reward3 ), sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total
from <TABLE>
where MapSize<>'40x40' and HasTimeLimit='no'
group by MapID,HiderCat3,SeekerType2,Reward3
order by MapID,HiderCat3,SeekerType2,Reward3"""]
,
["tab10: mapid",
"""select concat_ws(',',MapID), sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total
from <TABLE>
where MapSize<>'40x40' and HasTimeLimit='no'
group by MapID
order by MapID"""]
,
["tab10b: mapid,seeker,reward",
"""select concat_ws(',',MapID,SeekerType2,Reward3), sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total
from <TABLE>
where MapSize<>'40x40' and HasTimeLimit='no'
group by MapID,SeekerType2,Reward3
order by MapID,SeekerType2,Reward3"""]

]


qry_win_maps = """select concat_ws(',',OnOffline, SeekerType2,Reward3,MapSize), sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total
from <TABLE>
where MapSize='<MAPSIZE>'
group by OnOffline,SeekerType2,Reward3,MapSize"""
...
"""select concat_ws(',',OnOffline, SeekerType2, Reward3, MapID, IsWin), <COL>
from <TABLE>
where MapSize<>'40x40'"""]
]
...
["tab1b: on/off seek type, hider type",
"""select concat_ws(',',SeekerType2,Reward3,HiderCat3 ), <COL>
from <TABLE>
where IsWin=1 and MapSize='<MAPSIZE>' and HasTimeLimit='no'"""],
...
,
["tab9: on/off seek type, hider type + mapid",
"""select concat_ws(',',MapID,HiderCat3,SeekerType2,Reward3 ), <COL>
from <TABLE>
where IsWin=1 and MapSize='<MAPSIZE>'  and HasTimeLimit='no'"""]
,
["tab10: mapid",
"""select concat_ws(',',MapID), <COL>
from <TABLE>
where IsWin=1 and MapSize='<MAPSIZE>' and HasTimeLimit='no'"""]
,
["tab10b: mapid,seeker,reward",
"""select concat_ws(',',MapID,SeekerType2,Reward3),<COL>
from <TABLE>
where IsWin=1 and MapSize='<MAPSIZE>' and HasTimeLimit='no'"""]
