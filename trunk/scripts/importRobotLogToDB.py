...
#
# NOTE: quotes (") should be added to prevent bash/zsh to expand the joker command (*)
...
def importCSV(logfile):
	...
	try:
		...
		cur.execute("INSERT INTO RobotLog (ImportedTime,FileName,Info) VALUES (Now(),'%s','%s');" % (logfile,extraInfo) )
                # WARNING: since MAX is used, this script SHOULD NOT be used in parallel on the same DB
