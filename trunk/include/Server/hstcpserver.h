...
protected:
    ...
    void writeGameParamsToStream(QDataStream& out, QByteArray &byteArray, ClientSocket* client);
     ...
     */
    bool readNewAction(QDataStream& in, PlayerInfoServer* info);
    ...
    void writeUpdateToStream(QDataStream& out, QByteArray &byteArray,
                             std::vector<IDPos>& nextPosAutoWalkers);
    ...
    /*!
     * \brief applyNoiseHider add noise to hider pos, and check if visible, also based on dyn. obst. and distance.
     * If asDynObst=true, it is used to add noise for dyn. obst.
     * \param seekerPos seeker position
     * \param hiderPos position of the hider (or dyn. obst) which is added noise to
     * \param useDynObst (optional, default true) use dyn. obst. in the map to simulate occulting the hider
     * \param asDynObst (optional, default false) add noise to a dyn. obst. (uses different params)
     * \return
     */
    Pos applyNoiseHider(Pos seekerPos, Pos hiderPos, bool useDynObst=true, bool asDynObst=false);
    ...
    void writeRobotGoalsToStream(QDataStream &out, QByteArray &byteArray);
...
protected slots:
    ...
    void agentReadyRead(ClientSocket* client);
