...
struct SeekerHSParams {
    ...
    static const char SOLVER_FOLLOWER_SEES_ALL = 17; //AG151114: follower that sees all
    ...
    //AG160407: COMBI POMCP & Kalman
    //static const char SOLVER_COMBI_KALMAN_HIGHEST_BELIEF = 25;


    //AG150427: changed SOLVER_TWO_HB_EXPL to SOLVER_MULTI_HB_EXPL
    static const char SOLVER_TWO_HB_EXPL = 15;
    static const char SOLVER_MULTI_HB_EXPL = 16;
    //AG151114: Kalman Filter
    static const char SOLVER_FILTER_KALMAN = 18;
    //AG151126: kf requires that the person is always visible
    static const char SOLVER_FILTER_KALMAN_REQ_VISIB = 19;
    //AG160114: Particle Filter
    static const char SOLVER_FILTER_PARTICLE = 20;
    //AG160114: PF requires that the person is always visible
    static const char SOLVER_FILTER_PARTICLE_REQ_VISIB = 21;
    //AG151126: PF that always updates, even when 'no' observation
    static const char SOLVER_FILTER_PARTICLE_ALWAYS_UPDATES = 22;
    //AG151126: PF that always updates, even when 'no' observation, and uses Highest Belief and Follower when visible
    static const char SOLVER_FILTER_PARTICLE_ALWAYS_UPDATES_HB = 23;
    //AG160215: PF that always updates, even when 'no' observation, and uses multi belief explorer
    static const char SOLVER_MULTI_FILTER_ALWAYS_UPDATES_HB_PARTICLE = 24;

    //AG160407: COMBI PF and Kalman ??? OR USE KALMAN!!
    //static const char SOLVER_COMBI_KALMAN_HIGHEST_BELIEF = 26;
    ...
    */
    ...
    //AG161007
    //multi seeker obs. combiner
    static const char MULTI_SEEKER_OBS_COMB_MIN = 0;
    static const char MULTI_SEEKER_OBS_COMB_MAX = 1;
    static const char MULTI_SEEKER_OBS_COMB_AVG = 2;
    ...
    //HSSimulatorCont (AG160203: and PF)
    static constexpr double CONT_NEXT_SEEKER_STATE_STDDEV   = 0.2;  // 0.05;
    static constexpr double CONT_NEXT_HIDER_STATE_STDDEV    = 0.2; //AG161014: was 1.0; //AG160707, especially for big maps, was: 0.3;  //0.05;
    ...
    static constexpr double CONT_UPD_INCONSIST_ACCEPT_PROB  = 0.1; //AG161017: was 0.3, but too noisy;
    static constexpr double CONT_HIDER_HEAD_STDDEV          = 0.3;
    ...
    static constexpr double MSHB_MULTI_SEEKER_EXPLORER_EXT_SCORE_WEIGHT = 0; //AG160727:
    ...
    //Multi seeker process order
    static const char MSHB_PROC_ORDER_ID = 0;
    static const char MSHB_PROC_ORDER_BELIEF = 1;

    //AG160127
    //! default values for sim. not visib. prob.
    //!  p_visib =  y0                  if x<=x0
    //!             y0 + (x-x0)*k0      otherwise
    static constexpr double SIM_NOT_VISIB_DIST_DEFAULT_Y0 = 0.85;
    static constexpr double SIM_NOT_VISIB_DIST_DEFAULT_X0 = 3.0;
    static constexpr double SIM_NOT_VISIB_DIST_DEFAULT_K0 = -.17;
    //! default values for sim. not visib. prob. for autowalker (laser)
    static constexpr double SIM_NOT_VISIB_DIST_DEFAULT_DO_Y0 = 0.95;
    static constexpr double SIM_NOT_VISIB_DIST_DEFAULT_DO_X0 = 5.0;
    static constexpr double SIM_NOT_VISIB_DIST_DEFAULT_DO_K0 = -.19;

    //AG160211: TODO this might have to be a real variable
    //! max number of tries to find a legal position in case of incorrect position
    //! (used by POMCP simulator, Particle Filter, ..)
    static const unsigned int MAX_NUM_TRIES_AFTER_FAILURE = 100;
    ...
    //AG150518: number of players required by this player (0=no limit)
    //! number of players required
    unsigned int numPlayersReq;

    //! number of players in this client
    unsigned int numPlayersInClient;
    ...
    //AG160115:
    //! std. dev. heading noise
    double contHiderHeadStdDev;
    ...
    //! multi seeker process order (of the info of the seekers)
    char multiSeekerProcessOrder;
    //AG160725
    //! multi seeker explorer's weight of external score
    double multiSeekerExplorerExtScoreWeight;
    //! multi seeker explorer normalize with max distance
    bool multiSeekerExplorerExtScoreNormWMaxDist;
    //! external score mat file
    std::string multiSeekerExplorerExtScoreMatFile;

    //AG161007
    //! multi seeker observation combination
    char multiSeekerObsCombType;
    ...
    //AG160127
    //! simulate not visible using a probability based on the distance of the person/hider
    //! Note: this is not the same as, simulateNotVisible
    bool simNotVisibDist;

    //! parameters for simNotVisibDist:
    //!  p = y0                  if 0<x<=x0
    //!      min(y0-(x-x0)k0, 0) if x>x0
    double simNotVisibDist_x0, simNotVisibDist_y0, simNotVisibDist_k0;
    //! same for autowalker
    double simNotVisibDistDO_x0, simNotVisibDistDO_y0, simNotVisibDistDO_k0;

    //AG160216
    //! max distance to see something
    //!  y0 - (x-x0)k0 = 0
    //!  x = y0/k0 + x0
    double simNotVisibDist_maxX;

    //AG160505
    //! use received dyn. obst. as obst. in visib. checks used for learning (belief upd. etc.)
    bool useDynObstForVisibCheck;


    //AG150122
    //! check whether 2 seekers are used (uses gameType)
    bool seekerSendsMultiplePoses();

    //! number of players allowed/required for the game
    unsigned int maxNumberOfPlayers();

    //AG160216
    //! set the max distance params
    void setNotMaxVisDist(double cellSzM, double x0=SIM_NOT_VISIB_DIST_DEFAULT_X0, double y0=SIM_NOT_VISIB_DIST_DEFAULT_Y0, double k0=SIM_NOT_VISIB_DIST_DEFAULT_K0);
