 ...
 * The heading is not used for comparison.
 */
struct Pos {

    static constexpr double COMPARISON_EPSILON = std::numeric_limits<float>::epsilon();
    ...
    //AG150117
    /*!
     * \brief Pos create a pos based on an  Eigen vector wich should have at least 2 rows.
     * \param vec
     */
    Pos(const Eigen::Matrix<double, Eigen::Dynamic, 1>& vec);
     ...
     * \param ar
     * \param ac
     * \param canBeNegative allows negative values of the Pos
     */
    void add(double ar, double ac, bool canBeNegative=false);
     ...
     * \param canBeNegative allows negative values of the Pos
    */
    void add(int ar, int ac, bool canBeNegative=false);
    ...
    /*!
     * \brief getHash
     * \param useDouble (optional, default true) use double if available
     * \return
     */
    std::string getHash(bool useDouble=true) const;
    ...
    //AG150117
    /*!
     * \brief toEigenVector returns an Eigen vector with NROWS rows, and the values: vec(0)=rowDouble,vec(1)=colDouble
     * \param vec
     */
    template<int NROWS>
    Eigen::Matrix<double, NROWS, 1> toEigenVector() const {
    ...
    //AG150125: added heading

    //! get heading
    virtual double heading() const;

    //! has heading
    virtual bool hasHeading() const;
...
protected:
    ...
    //AG160125: private to be able to check correctness
    //! heading
    double _heading;
};
