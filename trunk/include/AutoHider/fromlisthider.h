...
public:
     ...
     * \param numSteps
     * \param useContinuous
     * \param othererLocFile
     * \param startVisibFromOther
     * \param distToOther
     */
    static void writePosFile(AutoWalker* autoWalker, GMap* map, std::string outFile, uint numHiders, uint numSteps, bool useContinuous,
                             std::string othererLocFile, bool startVisibFromOther, double distToOther);
...
protected:
    ...
    //AG160705
    //! show only one warning if failed to read more
    bool _warningDone;
};
