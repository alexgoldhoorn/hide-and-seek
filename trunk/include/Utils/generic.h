...
//TODO: should create a namespace for this utility functions
...
//AG160129: hsutils namespace for own utility functions
namespace hsutils {
...
/*!
 * \brief randomGauss random gaussian with mean and std. dev.
 * \param mean
 * \param std
 * \return
 */
double randomGauss(double mean, double std);
 ...
 * \return
 */
constexpr double sqrd(double x) {
    return x*x;
}

//AG160126
/*!
 * \brief dnnmod double-non-negative-modulus: uses std::fmod to calculate num % denum, with the resulting value
 * being [0,denom), also if num<0.
 * \param num
 * \param denom
 * \return
 */
double dnnmod(double num, double denom);
