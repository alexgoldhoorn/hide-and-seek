...
//ag120307: for import of DEBUG_LOG
//#include "Const.h"
//ag: ugly solution to copy, but didn't work including const
...
void SARSOP::solve(SharedPointer<MOMDP> problem)
{
	try
	{
		...
		elapsed = runtimeTimer.elapsed();
                DEBUG_LOG(printf("  initialization time : %.2fs\n", elapsed);
				...
				{//do check every CHECK_INTERVAL(50) backups
					...
					if (this->solverParams->interval > 0)
					{
						...
						if(currentElapsed > this->solverParams->interval)
						{
							...
							policyIndex++;
							elapsed += currentElapsed;
                                                        DEBUG_LOG(cout << "Intermediate policy written(interval: "<< this->solverParams->interval <<")" << endl;);
	...
	//now output policy to the outfile
        DEBUG_LOG(cout << endl << "Writing out policy ..." << endl;);
        DEBUG_LOG(cout << "  output file : " << this->solverParams->outPolicyFileName << endl;);
...
void SARSOP::initialize(SharedPointer<MOMDP> problem)
{
	printIndex = 0; // reset printing counter
        ...
        int xStateNum = problem->XStates->size(); //cout <<" xstatenm"<<xStateNum<<endl;
