...
void Function::fillTables(map<string, StateObsAct*> mymap,
                          vector<string> insttokens, vector<double> probttokensdouble) {
    bool simple = true;
    bool seestar = false;
    ...
    for (unsigned int i = 0; i < insttokens.size() && !seestar; i++) {
        ...
        if (insttokens[i] == "*" || insttokens[i] == "-") {
            seestar = true; // -> * and - (jokers) -> not simple
            simple = false;
            ...
            for (unsigned int j = 0; j < allCombinations.size(); j++) {
                ...
                vector<string> temp = cloneVectorList(insttokens);
    ...
    if (simple) {
        ...
        insttokens.erase(insttokens.begin() + insttokens.size() - 1);
        // Note insttokens size is now 1 less

        simpleSparseInsert(mymap, insttokens, myself, probttokensdouble.front());
    }
}

vector<double> Function::splitTable(vector<double> probttokensdouble, int size,
                                    int position) {
    vector<double> st;
...
vector<string> Function::cloneVectorList(vector<string> vectorList) {
    vector<string> newVectorList;
    for (unsigned int k = 0; k < vectorList.size(); k++) {
        string s(vectorList[k]);
        newVectorList.push_back(s);
    }
    return newVectorList;

}

void Function::simpleSparseInsert(map<string, StateObsAct*> mymap, vector<
                                  string> insttokens, string myself, double prob) {
    ...
    for (unsigned int i = 0; i < insttokens.size(); i++) {
        position = getPosition(mymap[parents[i]]->getValueEnum(), insttokens[i]);
        ci.push_back(position);
    }

    UniqueIndex ui;
    ui.index = getPosition(mymap[vnameCurr]->getValueEnum(), myself);
    ui.value = prob;
...
int Function::getPosition(vector<string> vlist, string str) {
