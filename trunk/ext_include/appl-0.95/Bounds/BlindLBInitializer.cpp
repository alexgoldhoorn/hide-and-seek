    ...
    FOR (a, pomdp->actions->size()) {
        ...
        do {
            ...
            FOR (sval, numStatesObs ) {	// for each sval, get nextAl[sval]
                ...
                FOR (nextSval, numStatesObs ) {
                    // commented out 22092008
                    /*		// if TX[a][sval] is all zero, dont do any multiplication, and dont need
                    // to add anything to tmpAl
                    copy_from_column( T_xa, pomdp->TX[a][sval], nextSval );
                    if (!T_xa.data.empty()) */

                    // ADDED new function .isColumnEmpty 22092008
                    if (!(pomdp->XTrans->getMatrix(a, sval)->isColumnEmpty(nextSval))) {
                        /*
                        //ag debug test 120303
                        SharedPointer<SparseMatrix> mat = pomdp->YTrans->getMatrix(a, sval, nextSval);
                        DenseVector dv = als[nextSval];
                        cout << "mat.sz1="<<mat->size1()<<",.sz2="<<mat->size2()<<",dv.sz="<<dv.size()<<flush;
                        mult(tmpNextAl, *mat, dv);
                              // SYL260809 mult(vector, matrix, vector) is faster than mult(vector, vector, matrix)
                              //mult(tmpNextAl, als[nextSval], *pomdp->XYTrans->matrixTr[a][sval]);

                        SharedPointer<SparseMatrix>  xmat=pomdp->XTrans->getMatrix(a, sval);
                        cout<<",xmat.sz1="<<xmat->size1()<<",.sz2="<<xmat->size2()<<",tmp.sz="<<tmpNextAl.size()<<flush;
                              emult_column( tmpNextAl1, *xmat, nextSval, tmpNextAl );
                              cout <<" t1.sz="<<tmpNextAl1.size()<<flush;
                              tmpAl += tmpNextAl1;
                              cout<<" tAl.sz="<<tmpAl.size()<<endl;*/
