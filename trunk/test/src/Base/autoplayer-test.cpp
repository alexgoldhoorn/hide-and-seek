...
int AutoPlayerTest::createPlayerInfoVec(std::vector<PlayerInfo *> &playerInfoVec,
    ...
    for(PlayerInfo* playerI : playerInfoVec) {
        ...
        if (playerI->isSeeker()) {
            ...
            if (hsutils::randomDouble()>OBS_HIDER_NOT_SEEN_P) // X% chance of setting pos
...
void AutoPlayerTest::updatePlayerInfoVec(std::vector<PlayerInfo *> &playerInfoVec) {
    for(size_t i=1; i<playerInfoVec.size(); i++) {
        ...
        if (!_params->seekerSendsMultiplePoses() || hsutils::randomDouble()<MULTISEEK2_CON_P) {
            ...
            if (pi->isSeeker() && hsutils::randomDouble()>OBS_HIDER_NOT_SEEN_P) { // X% chance of setting pos
...
void AutoPlayerTest::setMultiGoals2PlayerInfoVec(std::vector<PlayerInfo *> &playerInfoVec) {
    ...
    if (hsutils::randomDouble()<=MULTISEEK2_CON_P) {
        ...
        if (hsutils::randomDouble()<MULTISEEK2_GOAL_NOBEL_P) {
            otherPI->multiGoalBPosesVec[0].b=otherPI->multiGoalBPosesVec[1].b=-1;
        } else {
            otherPI->multiGoalBPosesVec[0].b = hsutils::randomDouble();
            otherPI->multiGoalBPosesVec[1].b = hsutils::randomDouble();
        }
