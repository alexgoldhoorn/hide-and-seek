...
void MCTSTest::testNodes(MCTS *mcts, GMap* map, Simulator* sim) {
    ...
    for(int q=0; q<10; q++) {
        ...
        NodeA* nodeAR = NULL;
        while (nodeAR==NULL) nodeAR = root->getChild( hsutils::random(root->childCount()-1) );
        Node* nodeR = nodeAR->getChildItem( hsutils::random(nodeAR->childCount()-1))->node;
