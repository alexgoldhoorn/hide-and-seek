...
double VerySmartHider2::scoreForState(const Pos &seekerPos, const Pos &hiderPos, const Pos &hiderPrevPos) { //  (int sr, int sc, int hr, int hc, int hrp, int hcp) {
    ...
    for(int sa=0; sa<HSGlobalData::NUM_ACTIONS; sa++) {
        ...
        //Pos sPos(sr,sc);
        Pos newSPos = _map->tryMove(sa,seekerPos);
        if (newSPos.isSet()) {
            //can move

            //calc score and update sum,min,max
            double as = score(newSPos, hiderPos); //  newSPos.row(), newSPos.col(), hr,hc,true); //we say visib because we take avg over all non-avg

            //AG131106: check for cross; AG150603: change to Pos
            //if (newSPos.row()==hrp && newSPos.col()==hcp && hr==sr && hc==sc) {
            if (newSPos==hiderPrevPos && seekerPos==hiderPos) {
                as -= _D2;
            }
...
double VerySmartHider2::score(const Pos &seekerPos, const Pos &hiderPos) { // (int sr, int sc, int hr, int hc, bool visib) {
    ...
    if (_scoreRandStd>0) {
        ...
        noise = ( dhs_factor*dhb_factor*_scoreRandStd * ( hsutils::randomDouble(0,2.0) /*2.0*rand()/RAND_MAX*/ - 1 ) ); //AG131113: noise with 0 as center
