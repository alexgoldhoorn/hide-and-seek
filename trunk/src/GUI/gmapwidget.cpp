...
GMapWidget::GMapWidget(bool useMouseObs, bool allowClickNextPos, QString metaInfo, QString comments, QWidget *parent) : QWidget(parent) {
    ...
    _showOppVisib = false;
    _mainWindow = NULL;
    _autoPlayerTimerTime_ms = AUTO_PLAY_DEFAULT_MS;

    setTitle("Hide-and-Seek Client");

    QTimer::singleShot(0, this, SLOT(showDialogForNewGame()));

    //AG160509: timer for auto play
    connect(&_autoPlayTimer, SIGNAL(timeout()), this, SLOT(autoPlayTimer()));
...
void GMapWidget::showDialogForNewGame() {
    ...
    if (_player!=NULL) {
        username = _player.playerInfo.getUserName(); //->getUsername();
    }*/
    string username = _player.playerInfo.username.toStdString();
    if (username.length()==0) {
        username = "test";
    }

    //create seeker params
    ...
    if (dialog.exec()) {
        ...
        //AG150603: clear the playerInfo struct
        _player.playerInfo.clear();
        //AG150604: clear player info vec
        _playerInfoVec.clear();

        //create connection object
        _connector = new GameConnectorClient(params, _metaInfo, _comments, &_player, mapToSend);
        ...
        connect(_connector,SIGNAL(serverUpdateReceived(int)),this,SLOT(handleServerUpdateReceived(int)));
...
bool GMapWidget::initPos(Pos initPos) {
    ...
    _player.playerInfo.currentPos = pos; // setCurPos(pos);
...
void GMapWidget::handleServerUpdateReceived(int gameStatus) {
    assert(_connector!=NULL);
    ...
    //AG150525: be sure to get the vector of players
    _playerInfoVec = _connector->getPlayerInfoVec();
...
//msg format: action, current position(x,y)
    ...
    if (_useMouseObs) { //AG130722: use mouse observation, ie user clicks observation
        ok = _connector->sendPos(&_mouseObs); // Action(NULL,&_mouseObs);
    } else {
        /*Pos p;
        p=_player.getCurPos();*/
        DEBUG_CLIENT(cout << "sending pos: "<< /*p*/ _player.playerInfo.currentPos.toString()<<endl;);
        ok = _connector->sendPos(); //Action(NULL,NULL);
    }
...
void GMapWidget::drawMap(QPainter& painter) {
    Pos playerPos = _player.playerInfo.currentPos;
    ...
    for (int r=0; r<_gmap->rowCount(); r++) {
        for (int c=0; c<_gmap->colCount(); c++) {
            ...
            //check if there is a map and the pos is set; AG160415: added isObstcle check
            if (_player.getMap()!=NULL && playerPos.isSet() && !_player.getMap()->isObstacle(r,c)) {
                isVisib = _gmap->isVisible(playerPos.row(), playerPos.col(), r, c, true, _gmap->getParams()->simNotVisibDist);
            }
            if (_connector!=NULL) {
    ...
    //AG150529: show current player also when no other players yet, if set
    // this is the case when the first location is shown
    if (playerPos.isSet()) {

        //AG150528: only draw players if there are any (otherwise either not connected yet or showing map only)
        if (!_playerInfoVec.empty()) {
            //draw dynamical obstacles
            vector<IDPos> dynObstVec;
            //AG160503: either see all, or see only the ones that the seeker can see
            if (_showAllPlayers)
                dynObstVec = _gmap->getDynamicObstacles();
            else
                dynObstVec = _player.playerInfo.dynObsVisibleVec;

            for(const IDPos& dPos: dynObstVec) {
                assert(!_gmap->isObstacle(dPos)); //AG160415: assume it is not an obtacle
                // bool visib = (_showAllPlayers || _debugMode || _gmap->isVisible(playerPos, dPos, true, _gmap->getParams()->simNotVisibDist) );
                //if (visib) {
            ...
            for (PlayerInfo* playerInfo : _playerInfoVec) {
                if (*playerInfo!=_player.playerInfo) {
                    //check visibility
                    bool visib = (_showAllPlayers || _gmap->isVisible(playerPos, playerInfo->currentPos, true, _gmap->getParams()->simNotVisibDist) );
                    if (visib) {
                        if (playerInfo->isSeeker()) {
                            drawPlayer(painter, playerInfo->currentPos, Qt::darkBlue, Qt::cyan, false, false, "s"+QString::number(playerInfo->id));
                        } else {
                            drawPlayer(painter, playerInfo->currentPos, !_connector->isSeeker(), false, false);
                        }
                    }
                }
            }
        ...
        //draw current player
        drawPlayer(painter, playerPos, _player.playerInfo.isSeeker(), !_showOppVisib, _connector->canDoAction());
...
void GMapWidget::keyPressEvent ( QKeyEvent * event ) {
    ...
    switch(event->key()) {
        ...
        case Qt::Key_Escape:{
            ...
            //AG151130: use Qt's quit() instead of exit() ! (otherwise segmentation fault)
            //exit(0);
            QApplication::quit();
    ...
    if (_debugMode) {
        switch(event->key()) {
            ...
            case Qt::Key_F:
                if (!_debugStartPos.isSet()){
                    ...
                    if (_debugPath.size()>1) {
                        ...
                        if (!nextPos.isSet()) { //AG170307: added
                            cout << "Warning! the path is not set"<<endl;
                            return;
                        }
            ...
            case Qt::Key_V:
                if (!_debugStartPos.isSet()){
                    ...
                    bool v = _gmap->isVisible(_debugStartPos, _debugEndPos, true, _gmap->getParams()->simNotVisibDist);
            ...
            case Qt::Key_Ampersand:
                ...
                _gmap->createPathPlanner(_neighborhood);
            ...
            case Qt::Key_Asterisk:
                ...
                break;
            case Qt::Key_4:
                if (_gmap->getPathPlanner()!=nullptr) {
                    cout << "ERROR: cannot set the neighborhood if the pathplanener is already set."<<endl;
                } else {
                    _neighborhood = 4;
                    cout << "Setting neighborhood to "<<_neighborhood<<endl;
                }
                break;
            case Qt::Key_8:
                if (_gmap->getPathPlanner()!=nullptr) {
        ...
        if (!_player.playerInfo.initPosSet /*!_connector->isMyInitPosSet()*/ ) { //if the initial position was not set yet from the hider then dont make a move yet.
            QMessageBox::information(this, "Set Position", "Please set your own position first by clicking on the map.");
            return;
        } else /*if (!_connector->isOppPosSet()) {  //AG150526: this check is not necessary, is like 'canDoAction'
        ...
        switch(event->key()) {
            ...
            case Qt::Key_A:
                if (_autoPlayTimer.isActive()) {
                    ...
                    _autoPlayTimer.start(_autoPlayerTimerTime_ms);
                }
                break;
            case Qt::Key_Z:
                if (_autoPlayerTimerTime_ms>AUTO_PLAY_CHANGE_MS)
                    _autoPlayerTimerTime_ms-=AUTO_PLAY_CHANGE_MS;
                cout << "Auto player timer time reduced to: "<<_autoPlayerTimerTime_ms<<" ms"<<endl;
                 break;
            case Qt::Key_Q:
                _autoPlayerTimerTime_ms+=AUTO_PLAY_CHANGE_MS;
                cout << "Auto player timer time increased to: "<<_autoPlayerTimerTime_ms<<" ms"<<endl;
                break;
            case Qt::Key_Backslash:
                //AG150120: test message
                QString msg="Test message";
                QByteArray msgBA=msg.toStdString().c_str();
                MessageSendTo to = (MessageSendTo)hsutils::random(0,6);
        ...
        if(madeMovement ) { //the action was made on the client
            //AG150602: set next pos, because Player.move() sets currentPos
            _player.playerInfo.nextPos = _player.playerInfo.chosenGoalPos = _player.playerInfo.currentPos;
...
void GMapWidget::setWindowTitleWithSteps() {
    ...
    // set title
    title.append(" (");
    title.append(_player.playerInfo.username);
    title.append(") - against: ");


    if(_connector->isSeeker()) {
        title.append("hider - with: ");
    }

    //AG150525: change to multi
    if (_playerInfoVec.size()>4) {
        title.append(QString::number(_playerInfoVec.size()-2));
        title.append(" seekers");
    } else {
        for (PlayerInfo* playerInfo : _playerInfoVec) {
            if (*playerInfo!=_player.playerInfo && !playerInfo->isSeeker()) {
                title.append(" ");
                title.append(playerInfo->username);
            }
        }
    }
    ...
    QString status = "Action "+QString::number(_player.playerInfo.numberActions /*getNumActions()*/) + "/";
    if (_connector->getParams()->maxNumActions) status += QString::number(_connector->getParams()->maxNumActions);
    setStatus(status);
}
...
void GMapWidget::showHelp() {
    ...
    if (!_onlyShowMap)
            ...
            << "Y / Up / 8      - go up/north<br/><br/>"<<endl<<endl
            << "A               - enable auto play<br/>"<<endl
            << "Q               - increase auto play wait time 500 ms<br/>"<<endl
            << "Z               - reduce auto play wait time "<<AUTO_PLAY_DEFAULT_MS<<" ms<br/>"<<endl<<endl;
...
void GMapWidget::mousePressEvent(QMouseEvent *e) {
    ...
    QPoint point = e->pos();
    Pos pos = WidgetToGMapPoint(point);

    //ag130722
    DEBUG_CLIENT(cout << "Clicked: "<< pos.toString()<<""<<endl;);
    setStatus(QString("Clicked ["+QString::fromLatin1(_gmap->isObstacle(pos)?"obstacle":"free")+"]:")+QString::fromStdString(pos.toString()));

    if (!_onlyShowMap && _connector!=NULL && _connector->haveServerParamsBeenRead() && /*!_connector->isMyInitPosSet()*/!_player.playerInfo.initPosSet) {
        if(!initPos(pos)) {
            return;
        }
        ...
        if (_gmap->isPosInMap(pos) && !_gmap->isObstacle(pos)) {
            ...
            _player.playerInfo.nextPos = pos;
...
void GMapWidget::autoPlayTimer() {
    //only do if can do action
    if (!_connector->canDoAction())
        return;
