...
GMap::GMap(SeekerHSParams* params) : _mapFixed(false), _editMode(false), _params(params), _cellSize_m(1.0)  {
...
GMap::GMap(int rows, int cols, SeekerHSParams* params) : _mapFixed(false), _editMode(false), _params(params), _cellSize_m(1.0) {
    createMap(rows,cols);
}

GMap::GMap(string fileName, SeekerHSParams* params) : _mapFixed(false), _editMode(false), _params(params), _cellSize_m(1.0)  {
    _name = fileName;
    readMapFile(fileName);
    setMapFixed();
}

GMap::GMap(string pgmFileName, const Pos& base, unsigned int zoomOutFactor, SeekerHSParams* params) : _mapFixed(false), _editMode(false),
                        _params(params), _cellSize_m(1.0)  {
    _name = pgmFileName;
    readPGMMapFile(pgmFileName,base,zoomOutFactor);
}

GMap::GMap(string yamlFileName, const Pos& base, double cellSize, double rowDist_m, double colDist_m, SeekerHSParams *params) {
    _name = yamlFileName;
    _params = params;
...
void GMap::readMapFile(string fileName) {
    ...
    if (reader.good()) {
        ...
        //AG160503: check cell size, is an optional parameter, default 1.0
        string::size_type i3 = line.find(",",i2+1);
        if (i3!=string::npos) {
            string cstr = line.substr(i2+1,i3-i2+1);
            _cellSize_m = atof(cstr.c_str());
        }

        //set cell size and params
        if (_params!=nullptr)
            _params->setNotMaxVisDist(_cellSize_m);

        DEBUG_MAP(cout <<_rows<<" x "<<_cols<<" (cell size="<<_cellSize_m<<" m)"<<endl<<"creating map... "<<flush;);
        ...
        exit(EXIT_FAILURE);
        cout << "ERROR @ GMap::readMapFile: Map file "<<fileName<<" could not be opened."<<endl;
    }
}
...
void GMap::writeMapFile(string fileName) {
    ...
    writer << _cols << "," << _rows << "," << _cellSize_m << "," <<endl;
    for(int r=0; r<_rows; r++) {
        for(int c=0; c<_cols; c++) {
            writer << (int)_map[r][c];
        }
...
void GMap::printMap(bool showDynObst, int sr,int sc, int hr, int hc, std::ostream& stream) {
    //AG130228
    bool useVisib = sr>=0;


    if (useVisib) stream <<" =visible free; ·=invisible free; capital=visible, small=invisible"<<endl;
    ...
    for (int r=-2; r<=_rows; r++) {
        for (int c=-2; c<=_cols; c++) {
            ...
            if (r<0 || r==_rows) {
                ...
                bool isInVis = false;
                if (useVisib) isInVis = !isVisible(sr,sc,r,c,showDynObst,_params->simNotVisibDist); //ag130301: use function, since it generates the visibility map
...
void GMap::printMap(const Pos& p, bool showDynObst, std::ostream& stream) {
    printMap(showDynObst, p._row,p._col, -1, -1, stream);
}

void GMap::printMap(const Pos& seekerPos, const Pos& hiderPos, bool showDynObst, std::ostream& stream) {
    printMap(showDynObst, seekerPos._row, seekerPos._col, hiderPos._row, hiderPos._col, stream);
}

void GMap::createPathPlanner(int neighborhood) {
    if (_pathPlanner!=NULL) {
    ...
    switch(_params->pathPlannerType) {
        case SeekerHSParams::PATHPLANNER_PROPDIST:
            _pathPlanner = new PropDistPlanner(_map, _rows, _cols,neighborhood);
            break;
        case SeekerHSParams::PATHPLANNER_ASTAR:
            _pathPlanner = new AStarPlanner(_map, _rows, _cols,neighborhood);
            break;
        default:
...
/*bool GMap::isVisible(int r1, int c1, int r2, int c2, bool checkDynObst, bool useMaxVisDist, const vector<IDPos>* dynObstVec) {
    assert(_mapFixed);
    assert(dynObstVec==nullptr || dynObstVec->empty() || !checkDynObst);
    //check map coordinates
    assert(r1>=0 && c1>=0 && r1<_rows && c1<_cols);
    assert(r2>=0 && c2>=0 && r2<_rows && c2<_cols);

    if (_visibleMat == NULL) {
        //create cache (initialized with VMAT_UNSET)
        initVisMat();
    }
    ...
    //AG160216: check if it passed the max distance
    if (isVisib && useMaxVisDist && _params->simNotVisibDist_maxX>0) {
        assert(_params->simNotVisibDist_maxX>1.0);
        isVisib = ((distanceEuc(r1,c1,r2,c2) <= _params->simNotVisibDist_maxX));
    }

    //AG160503: check dyn. obst. passed or all
    if ( isVisib && (( checkDynObst && !_dynObstVector.empty() ) || (dynObstVec!=nullptr && !dynObstVec->empty())) ) {
        //AG140525: check if visible due to dyn. obst
        if (_visibleMatDynObst==NULL) {
            _visibleMatDynObst = AllocateDynamicArray<char>(_rows,_cols);
        }

        //AG160415: removed if(!_lastVisib...), because the dyn. obstacles have moved,
        // therefore we have to update the map completely
        //if (!_lastVisibOrigPos.isSet() || !_lastVisibOrigPos.equalsInt(r1,c1)) {
            //set all 'visible' to unset and set original obstacles
            FOR(r,_rows) {
                FOR(c,_cols) { //TODO: CHECK IS THIS CORRECT??? WE STILL HAVE THE PREVIOUS POSITION SET TO BE OBSTACLE!
            ...
            } else if (dynObstVec!=nullptr){
                //passed dyn. obst.
                for(const IDPos& pos : *dynObstVec) {
                    _visibleMatDynObst[pos.row()][pos.col()] = VMAT_OBST;
                }
...
bool GMap::isVisible(double r1d, double c1d, double r2d, double c2d, bool checkDynObst, bool useMaxVisDist, const vector<IDPos>* dynObstVec) {
    assert(_mapFixed);
    assert(dynObstVec==nullptr || dynObstVec->empty() || !checkDynObst);
    //ag160503: convert to int
    int r1 = (int)floor(r1d);
    int c1 = (int)floor(c1d);
    int r2 = (int)floor(r2d);
    int c2 = (int)floor(c2d);
    //check map coordinates
    assert(r1>=0 && c1>=0 && r1<_rows && c1<_cols);
    assert(r2>=0 && c2>=0 && r2<_rows && c2<_cols);
    ...
    if (isVisib && useMaxVisDist && _params->simNotVisibDist_maxX>0) {
        assert(_params->simNotVisibDist_maxX>1.0);
        isVisib = ((distanceEuc(r1d,c1d,r2d,c2d) <= _params->simNotVisibDist_maxX));
    }
    ...
    if ( isVisib && (( checkDynObst && !_dynObstVector.empty() ) || (dynObstVec!=nullptr && !dynObstVec->empty())) ) {
        ...
        //if (!_lastVisibOrigPos.isSet() || !_lastVisibOrigPos.equalsInt(r1,c1)) {
            ...
            if (checkDynObst) {
                //all
                for(const IDPos& pos : _dynObstVector) {
                    assert(isPosInMap(pos) && !isObstacle(pos));
                    _visibleMatDynObst[pos.row()][pos.col()] = VMAT_OBST;
                }
            } else if (dynObstVec!=nullptr){
                //passed dyn. obst.
...
//bool GMap::isVisible(double r1, double c1, double r2, double c2, bool checkDynObst, bool useMaxVisDist, const vector<IDPos>* dynObstVec) {
/*bool GMap::isVisible(int r1, int c1, int r2, int c2, bool checkDynObst, bool useMaxVisDist, const vector<IDPos>* dynObstVec) {
    //return isVisible((int)floor(r1),(int)floor(c1),(int)floor(r2),(int)floor(c2), checkDynObst, useMaxVisDist, dynObstVec);
    return isVisible(r1,c1,r2,c2,checkDynObst, useMaxVisDist, dynObstVec);
}*/

bool GMap::isVisible(const Pos& p1, const Pos& p2, bool checkDynObst, bool useMaxVisDist, const vector<IDPos>* dynObstVec) {
    return isVisible(p1._rowDouble,p1._colDouble,p2._rowDouble,p2._colDouble, checkDynObst, useMaxVisDist, dynObstVec);
}
...
char GMap::raytrace(char **m, int x0, int y0, int x1, int y1)
{
    ...
    //AG151223
    if (m[x0][y0]==VMAT_OBST) {
        //throw CException(_HERE_, "Cannot check visibility from an obstacle.");
        //AG160415: this happens when dyn. obstacles are used!!
        return VMAT_NOT_VIS;
    }
...
vector<Pos> GMap::getInvisiblePoints(const Pos& p, bool checkDynObst, bool useMaxVisDist, const vector<IDPos>* dynObstVec) {
    return getInvisiblePoints(p._row, p._col,checkDynObst, useMaxVisDist, dynObstVec);
}

vector<Pos> GMap::getInvisiblePoints(int r, int c, bool checkDynObst, bool useMaxVisDist, const vector<IDPos>* dynObstVec) {
    assert(r>=0 && c>=0 && r<_rows && c<_cols);

    vector<Pos> posVector;
    FORs(r2,_rows) {
        FORs(c2,_cols) {
            if ( _map[r2][c2]!=GMAP_OBSTACLE && !isVisible(r,c,r2,c2,checkDynObst,useMaxVisDist, dynObstVec)) {
...
vector<Pos> GMap::getInvisiblePoints(const std::vector<Pos> &posVecIn, bool checkDynObst, bool useMaxVisDist, const vector<IDPos>* dynObstVec) {
    vector<Pos> posVector;
    FORs(r2,_rows) {
        FORs(c2,_cols) {
            if ( _map[r2][c2]!=GMAP_OBSTACLE) {
                //check visibility from all poses
                bool isVisib = false;
                for(const Pos& pos : posVecIn) {
                    if (isVisible(pos.row(),pos.col(),r2,c2,checkDynObst, useMaxVisDist, dynObstVec)) {
                        //visible
                        isVisib = true;
                        break;
                    }
...
double GMap::getVisibilityProb(const Pos &fromPos, const Pos &pos, bool checkDynObst, const vector<IDPos>* dynObstVec) {
    double p = 0;

    if (isVisible(pos, fromPos, checkDynObst,_params->simNotVisibDist,dynObstVec)) {
        if (_params->simNotVisibDist) {
            //calculate probability of visibility based on distance
            double d = fromPos.distanceEuc(pos);
            p = _params->simNotVisibDist_y0;
            if (d > _params->simNotVisibDist_x0)
                p += _params->simNotVisibDist_k0 * (d-_params->simNotVisibDist_x0);
...
double GMap::getVisibilityProb(const std::vector<Pos> &fromPosVec, const Pos &pos, bool checkDynObst, const vector<IDPos>* dynObstVec) {
    //calculate overal p
    // this is: P(X1=1 or X2=1 or...) = 1 - (1-P(X=1))(1-P(X=2))...

    double pNotVisib = 1.0;
    for(const Pos& fromPos : fromPosVec) {
        double p = getVisibilityProb(fromPos, pos, checkDynObst, dynObstVec);
        pNotVisib *= (1.0 - p);
    }

    double pVisib = 1.0 - pNotVisib;
    assert(pVisib>=0 && pVisib<=1.0);
    return pVisib;
}
...
Pos  GMap::tryMoveDir(double dir, Pos pos, double dist, bool visibCheck) {
    ...
    if (!isPosInMap(newPos) || isObstacle(newPos) || (visibCheck && !isVisible(pos,newPos,false,false)) //AG140526 note: the dyn. obst is NOT used as obstacle in nav.
#ifdef DYN_OBST_COLL
...
Pos  GMap::tryMoveDirStep(double dir, Pos pos, double dist, double stepSize, bool visibCheck) {
    ...
    do {
        ...
        continueSearch = (!isPosInMap(newPos) || isObstacle(newPos) || (visibCheck && dist>1 && !isVisible(pos,newPos,false,false)) )
...
void GMap::setUsingContActions(bool b) {
    ...
    if (b && _base.isSet() && _base._row==_base._rowDouble && _base._col==_base._colDouble) {
...
bool GMap::writeToSharedMemory(QSharedMemory &shMem) {
    //first check if distance matrix is set
    if (_distMat==NULL) {
        cout << "GMap::writeToSharedMemory: ERROR distance matrix is not set"<<endl;
        return false;
    }
    for(int r1=0; r1<_rows; r1++) {
        for(int c1=0; c1<_cols; c1++) {
            for(int r2=0; r2<_rows; r2++) {
                for(int c2=0; c2<_cols; c2++) {
                    if (_distMat[r1][c1][r2][c2]==DMAT_UNSET) {
                        cout << "GMap::writeToSharedMemory: ERROR distance matrix is not completely set (r"<<r1<<"c"<<c1
                             <<"-r"<<r2<<"c"<<c2<<")"<<endl;
                        return false;
                    }
                }
...
bool GMap::readFromSharedMemory(QSharedMemory &hsMem) {
...
double GMap::getCellSize_m() const {
    return _cellSize_m;
}

void GMap::setCellSize_m(double cellSize_m) {
    _cellSize_m = cellSize_m;
}
...
Pos GMap::genRandomPos() const {
    ...
    do {
        if (_params->useContinuousPos) {
            ranPos.set(hsutils::randomDouble(0,_rows), hsutils::randomDouble(0,_cols));
        } else {
            ranPos.set(hsutils::random(0,_rows-1), hsutils::random(0,_cols-1));
        }
