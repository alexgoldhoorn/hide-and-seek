...
SeekerHSParams::SeekerHSParams() {
    ...
    //std
    contNextSeekerStateStdDev = CONT_NEXT_SEEKER_STATE_STDDEV;
    contNextHiderStateStdDev = CONT_NEXT_HIDER_STATE_STDDEV;
    contSeekerObsStdDev = CONT_SEEKER_OBS_STDDEV;
    contHiderObsStdDev = CONT_HIDER_OBS_STDDEV;
    contFalsePosProb = CONT_FALSE_POS_PROB;
    contFalseNegProb = CONT_FALSE_NEG_PROB;
    contObserveIfNotVisibProb = CONT_OBS_IF_NOT_VISIB_PROB;
    contIncorPosProb = CONT_INCOR_POS_PROB;
    contUpdInconsistAcceptProb = CONT_UPD_INCONSIST_ACCEPT_PROB;
    contHiderHeadStdDev = CONT_HIDER_HEAD_STDDEV;
    //AG160330: set as minium the std.dev. for 3*(step + observation) since this should give the minimum of 99.7% of the cases
    //  --> now resp. 3*(0.2+0.1)=0.9, and 3*(0.3+0.1)=1.2
    contConsistCheckSeekerDist = 3.0 * (CONT_NEXT_SEEKER_STATE_STDDEV + CONT_SEEKER_OBS_STDDEV );//0;
    contConsistCheckHiderDist  = 3.0 * (CONT_NEXT_HIDER_STATE_STDDEV  + CONT_HIDER_OBS_STDDEV  );//0;
    //AG140908
    contNextHiderHaltProb = CONT_NEXT_HIDER_HALT_PROB;
    contUseHiderPredStepProb = CONT_USE_HIDER_PRED_STEP_PROB;
    ...
    multiSeekerExplorerExtScoreMatFile = "";
    multiSeekerExplorerExtScoreNormWMaxDist = false;
    multiSeekerExplorerExtScoreWeight = 0;
    ...
    multiSeekerObsCombType = MULTI_SEEKER_OBS_COMB_AVG;
    ...
    //max distance visible
    simNotVisibDist = true;
    setNotMaxVisDist(cellSizeM);

    //AG160505
    useDynObstForVisibCheck = true;
}
...
void SeekerHSParams::printVariables(ostream& stream, bool showExtraParams) {
    ...
    switch(solverType) {
        ...
        case SOLVER_FILTER_KALMAN_REQ_VISIB:
            ...
            break;
        case SOLVER_FILTER_PARTICLE:
            stream << "Particle Filter"<<endl;
            break;
        case SOLVER_FILTER_PARTICLE_REQ_VISIB:
            stream << "Particle Filter (req. visibility)"<<endl;
            break;
        case SOLVER_FILTER_PARTICLE_ALWAYS_UPDATES:
            stream << "Particle Filter (always updates)"<<endl;
            break;
        case SOLVER_FILTER_PARTICLE_ALWAYS_UPDATES_HB:
            stream << "Combined: Highest Bel. Particle Filter (always upd.) & Follower"<<endl;
            break;
        case SOLVER_MULTI_FILTER_ALWAYS_UPDATES_HB_PARTICLE:
            stream << "Multi Seeker Highest Bel. Particle Filter (always upd.)"<<endl;
            break;
        default:
            stream <<"Unknown"<<endl;
    }
    ...
    stream    << "Num. players this cl.:  "<<numPlayersInClient<<endl
              ...
              << "Use heading:            "<<(useHeading?"yes":"no")<<endl
              << "Path planner:           ";
    ...
    if (solverType==SOLVER_POMCP || solverType==SOLVER_COMBI_POMCP_FOLLOWER
            || solverType==SOLVER_COMBI_FOLLOWER_HIGHEST_BELIEF
            || solverType==SOLVER_FOLLOWER_HIGHEST_BELIEF || solverType==SOLVER_TWO_HB_EXPL
            || solverType==SOLVER_MULTI_HB_EXPL || solverType==SOLVER_FILTER_KALMAN
            || solverType==SOLVER_FILTER_KALMAN_REQ_VISIB || solverType==SOLVER_FILTER_PARTICLE
            || solverType==SOLVER_FILTER_PARTICLE_REQ_VISIB
            || solverType==SOLVER_MULTI_FILTER_ALWAYS_UPDATES_HB_PARTICLE ) {


        stream << "POMCP learning:         " << (pomcpDoNotLearn?"DISABLED - only belief update":"enabled")<<endl;

        if (solverType==SOLVER_FILTER_PARTICLE || solverType==SOLVER_FILTER_PARTICLE_REQ_VISIB ) {
            stream << "Numb. of Particles:     ";
        } else {
            stream << "Numb. belief states:    ";
        }
        stream << (numBeliefStates<=0?"[not set] ":"")<<numBeliefStates<<endl;

        stream << "Number of simulations:  " << (numSim<=0?"[not set] ":"")<<numSim<<endl
        ...
        if (pomcpSimType==POMCP_SIM_CONT || pomcpSimType==POMCP_SIM_PRED /*solverType==SOLVER_POMCP_CONT*/
                || solverType==SOLVER_FILTER_KALMAN
                || solverType==SOLVER_FILTER_KALMAN_REQ_VISIB
                || solverType==SOLVER_FILTER_PARTICLE
                || solverType==SOLVER_FILTER_PARTICLE_REQ_VISIB) {
            ...
            stream    << "Next seeker state stdev:" << contNextSeekerStateStdDev <<endl
                      ...
                      << "Cont Hider Head std.dev:" << contHiderHeadStdDev << endl
        ...
        stream << "POMCP filter belief upd:" << (pomcpFilterBeliefAtUpdate?"yes":"no") <<endl
               << "Sim. not visib. dist.:  ";
        if (simNotVisibDist) {
            stream << "yes (x0="<<simNotVisibDist_x0<<",y0="<<simNotVisibDist_y0<<",k0="<<simNotVisibDist_k0<<",maxDist="<<simNotVisibDist_maxX<<")"<<endl;
        } else {
            stream << "no"<<endl;
        }
        ...
        if (seekerSendsMultiplePoses()) {
            stream        << "Multi seeker own obs pr:" <<multiSeekerOwnObsChooseProb << endl
                          ...
                          << "Multi s. expl. ext. w.: " << multiSeekerExplorerExtScoreWeight << endl
                          << "Multi s. expl. ext.file:" << multiSeekerExplorerExtScoreMatFile << endl
                          << "Multi s. expl. ext.norm:" << multiSeekerExplorerExtScoreNormWMaxDist << endl
            ...
            stream    << "Multi s. obs comb. type:";
            switch (multiSeekerObsCombType) {
                case MULTI_SEEKER_OBS_COMB_AVG:
                    stream << "average"<<endl;
                    break;
                case MULTI_SEEKER_OBS_COMB_MIN:
                    stream << "minimum"<<endl;
                    break;
                case MULTI_SEEKER_OBS_COMB_MAX:
                    stream << "maximum"<<endl;
                    break;
            }
        }
    ...
    if (solverType==SOLVER_COMBI_FOLLOWER_HIGHEST_BELIEF || solverType==SOLVER_FOLLOWER_HIGHEST_BELIEF || solverType==SOLVER_TWO_HB_EXPL ||
            solverType==SOLVER_FILTER_PARTICLE_ALWAYS_UPDATES_HB || solverType==SOLVER_MULTI_FILTER_ALWAYS_UPDATES_HB_PARTICLE ||
            solverType==SOLVER_MULTI_HB_EXPL) {
    ...
    stream  << "Only send sim. steps:   " << (onlySendStepGoals?"yes":"no")<<endl
            ...
            << "Use dyn.obst. visib.chk:" << (useDynObstForVisibCheck?"yes":"no")<<endl
#ifdef DYN_OBST_COLL
            ...
            << "Time:                   " << hsutils::currentTimeStamp() << endl
            << endl<<endl;
}
...
void SeekerHSParams::setNotMaxVisDist(double cellSzM, double x0, double y0, double k0) {
    cellSizeM = cellSzM;
    //convert to cell size values
    simNotVisibDist_x0 = x0/cellSizeM;
    simNotVisibDist_y0 = y0;
    simNotVisibDist_k0 = k0*cellSizeM;
    //AG160504: dyn obst values
    simNotVisibDistDO_x0 = SIM_NOT_VISIB_DIST_DEFAULT_DO_X0/cellSizeM;
    simNotVisibDistDO_y0 = SIM_NOT_VISIB_DIST_DEFAULT_DO_Y0;
    simNotVisibDistDO_k0 = SIM_NOT_VISIB_DIST_DEFAULT_DO_K0*cellSizeM;

    //Max distance
    simNotVisibDist_maxX = simNotVisibDist_x0 - simNotVisibDist_y0 / simNotVisibDist_k0;
}
