...
void SeekerHSLog::open(string gamelogFile) {
    ...
    if (!exists) {
        ...
        string header = "step_num,exp_name,seeker1_x,seeker1_y,hiderO1_x,hiderO1_y,seeker2_x,seeker2_y,hiderO2_x,hiderO2_y,"                //logline
                        ...
                        "seeker1_numGoals_HBs,seeker2_numGoals_HBs,seeker_reward,num_dyn_obst1,num_dyn_obst2"; //,seeker_beliefScore";
...
void SeekerHSLog::stopGame(int winState) {
    if (_gamelog!=nullptr) {
        ...
        cout << "SeekerHSLog::stopGame: WARNING game stopped, but gameLog was not opened yet."<<endl;
    }
}
...
void SeekerHSLog::logLine2(const PosXY *chosenGoalPosXY) {
    ...
    if (_thisSeekerPlayerInfo->multiHasGoalPoses) {
        ...
        if (_otherSeekerPlayerInfo!=NULL && _otherSeekerPlayerInfo->multiHasHBPoses) { //AG160314: was ||
            _gamelog->print(_thisSeekerPlayerInfo->multiHBPosVec.size());
        } else {
            _gamelog->print("_");
        }
    ...
    _gamelog->print(_thisSeekerPlayerInfo->seekerReward);

    //AG160510: number of dyn. obst
    _gamelog->print(_thisSeekerPlayerInfo->dynObsVisibleVec.size());
    if (_otherSeekerPlayerInfo!=NULL) {
        _gamelog->printLine(_otherSeekerPlayerInfo->dynObsVisibleVec.size());
    } else {
