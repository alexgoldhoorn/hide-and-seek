...
void GameConnectorClient::sendInfoToServer() {
    ...
    //name
    out << QString::fromStdString(_params->userName);
    ...
    //AG160216: sim. not visib based on dist
...
void GameConnectorClient::readInitInfoFromServer(QDataStream& in) {
    ...
    //AG160414: exp. name
    in >> expName;
    _params->expName = expName.toStdString();
    ...
    //AG160524: read dist map file
    QString mapDistFileStr;
    in >> mapDistFileStr;
    _params->mapDistanceMatrixFile = mapDistFileStr.toStdString();
    ...
    in >> _params->simNotVisibDist;
    //AG160505: use dyn. obst for visib check
    in >> _params->useDynObstForVisibCheck;
...
void GameConnectorClient::readServerUpdate(QDataStream &in) {
    ...
    for (quint8 i=0; i<numPlayersS; i++) {
        ...
        //AG160503: now read visible autowalkers
        quint16 numVisibAutoW;
        in >> numVisibAutoW;
        //cout<<" VISIBLE AUTOW FOR "<<i<<" #"<<numVisibAutoW<<":"<<endl;
        _playerInfoVec[pid]->dynObsVisibleVec.resize(numVisibAutoW);
        for(IDPos& p :_playerInfoVec[pid]->dynObsVisibleVec) {
    ...
    if(_gameStatus > HSGlobalData::GAME_STATE_RUNNING) {
        ...
        if (!_params->beliefImageFile.empty()
            ...
            storeMapBeliefAsImage(_autoPlayerVec[0]);
        }
...
void GameConnectorClient::initBelief() {
    ...
    if (_params->isSeeker) {
        ...
        if (_seekerHS!=NULL) {      //TODO: change init belief, and ONLY use seekersh (?)
            ...
            PosXY seekerPosXY;
            PosXY hiderObsPosXY;
            vector<PosXY> dynObstPosXYVec;
            vector<PosXY> otherSeekerPosXYVec;
            vector<PosXY> otherSeekerHiderObsPosXYVec;
            vector<vector<PosXY>> otherDynObstPosXYVecVec;
            getSeerkHSPosXYForAll(seekerPosXY, hiderObsPosXY, dynObstPosXYVec, otherSeekerPosXYVec, otherSeekerHiderObsPosXYVec, otherDynObstPosXYVecVec);

            //AG150202: check if two seekers
            if (_params->seekerSendsMultiplePoses()) {
                _seekerHS->initMultiSeeker(seekerPosXY, hiderObsPosXY, dynObstPosXYVec, otherSeekerPosXYVec, otherSeekerHiderObsPosXYVec, otherDynObstPosXYVecVec);

            } else {
                //_autoPlayer->initBelief(_gmap,curPos,oppPos,isVisib);
                //_seekerHS->init(curPos.toVector(),ov);
                _seekerHS->init(seekerPosXY, hiderObsPosXY, dynObstPosXYVec);
            }
        } else
#endif
        ...
        switch(_params->solverType) {
            case SeekerHSParams::SOLVER_TWO_HB_EXPL: {
                ...
                if (otherSeekerPosXYVec.empty()) {
                    ...
                    _seekerHS->calcNextMultiRobotPoses2(seekerPosXY, hiderObsPosXY, dynObstPosXYVec, nullptr, nullptr, nullptr,
                        goal1PosXY, goal2PosXY, &winState);
                } else {
                    assert(otherSeekerHiderObsPosXYVec.size()==otherSeekerPosXYVec.size());
                    _seekerHS->calcNextMultiRobotPoses2(seekerPosXY, hiderObsPosXY, dynObstPosXYVec, &otherSeekerPosXYVec[0], &otherSeekerHiderObsPosXYVec[0],
                        &otherDynObstPosXYVecVec[0], goal1PosXY, goal2PosXY, &winState);
                }
            ...
            case SeekerHSParams::SOLVER_MULTI_HB_EXPL: {
                ...
                if (otherSeekerPosXYVec.empty()) {
                    assert(otherSeekerHiderObsPosXYVec.empty());
                    _seekerHS->calcMultiHBs(seekerPosXY, hiderObsPosXY, dynObstPosXYVec, nullptr, nullptr, nullptr, hbVec, &winState);
                } else {
                    assert(otherSeekerHiderObsPosXYVec.size()==otherSeekerPosXYVec.size());
                    _seekerHS->calcMultiHBs(seekerPosXY, hiderObsPosXY, dynObstPosXYVec, &otherSeekerPosXYVec[0], &otherSeekerHiderObsPosXYVec[0],
                        &otherDynObstPosXYVecVec[0], hbVec, &winState);
                }
            ...
            default: {
                ...
                int winState=-1;
                int a = _seekerHS->getNextPos(seekerPosXY, hiderObsPosXY, dynObstPosXYVec, newSeekerPosXY, &winState);
            ...
            if (_params->simulateReceivingAllTracksAsPersons) {
                ...
                if (_params->simulateNotVisible) {
                    ...
                    for(size_t i=0; i<personVec.size(); i++) {
                        if (!_gmap->isVisible(playerInfo->currentPos, personVec[i], false,_params->simNotVisibDist)) {
...
void GameConnectorClient::readSeekerHB(QDataStream &in) {
    ...
    if (_seekerHS==NULL) {
        ...
        if (!_params->multiSeekerNoCommunication && _params->numPlayersReq>2) {
            assert(_seeker2Player!=NULL);
...
void GameConnectorClient::onServerDisconnect() {
    ...
    if (_exitOnDisconnect) {
        cout << "EXITING (server disconnect)"<<endl;
...
#ifndef DO_NOT_USE_SEEKERSH_IN_GAMECONNECTOR
void GameConnectorClient::getSeerkHSPosXYForAll(PosXY &thisSeekerPosXY, PosXY &thisSeekerHiderObsPosXY, std::vector<PosXY>& thisDynObstPosXYVec,
                                                std::vector<PosXY>& otherSeekerPosXYVec, std::vector<PosXY>& otherSeekerHiderObsPosXYVec,
                                                std::vector<std::vector<PosXY>>& otherDynObstPosXYVecVec) {
    assert(_seeker1Player->id>=0);
    ...
    assert(otherSeekerHiderObsPosXYVec.empty());
    assert(otherSeekerPosXYVec.empty());
    assert(thisDynObstPosXYVec.empty());
    assert(otherDynObstPosXYVecVec.empty());
    ...
    //AG160510: dyn. obst.
    thisDynObstPosXYVec.resize(_seeker1Player->dynObsVisibleVec.size());
    for(size_t i=0; i<thisDynObstPosXYVec.size(); i++) {
        thisDynObstPosXYVec[i].fromPos(_seeker1Player->dynObsVisibleVec[i], _params);
    }
    ...
    for(PlayerInfo* pInfo : _playerInfoVec) {
        if (pInfo->isSeeker() && *pInfo!=*_seeker1Player) {
            ...
            //AG160510: dyn. obst.
            vector<PosXY> dynObstPosXYVec(pInfo->dynObsVisibleVec.size());
            for(size_t i=0; i<dynObstPosXYVec.size(); i++) {
                dynObstPosXYVec[i].fromPos(pInfo->dynObsVisibleVec[i], _params);
            }
            otherDynObstPosXYVecVec.push_back(dynObstPosXYVec);
        }
    }
}
#endif

void GameConnectorClient::storeMapBeliefAsImage(AutoPlayer *autoPlayer) {
    assert(autoPlayer!=nullptr);
    ...
    cout << "STORING ..."<<endl;
        ...
        if (beliefImgQStr.contains("#")) {
            ...
            beliefImgQStr = beliefImgQStr.replace("#",QString::number(autoPlayer->playerInfo.numberActions));
            beliefImg = beliefImgQStr.toStdString();
        }
#endif
        cout << "Stroring to " << beliefImg << endl;
    autoPlayer->storeMapBeliefAsImage(beliefImg, _params->beliefImageCellWidth);
}
