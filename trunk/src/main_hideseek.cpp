...
//see: http://stackoverflow.com/questions/13878373/where-am-i-supposed-to-reimplement-qapplicationnotify-function
/*
class MyQApplication : private QCoreApplication {
public:
    MyQApplication(int &argc, char* argv[]) : QCoreApplication(argc, argv) {
    }
private:
    bool notify(QObject *receiver_, QEvent *event_) {
        try {
            ...
            cout << "MyQApplication::notify: "<< e.what()<<endl;
            return false;
        }
    }
            ...
            switch (arg[1]) {
                    ...
                    if (strlen(arg)>2) {
                        switch (arg[2]) {
                            ...
                            case 'o':
                                if (i+1<argc && argv[i+1][0]!='-') {
                                    //expected the timout
                                    QString toStr(argv[++i]);
                                    bool ok = false;
                                    checkConnWaitTime_msec = toStr.toInt(&ok)*1000;
                                    if (!ok) showHelp("expected timeout (s) instead of ",EXIT_FAILURE,argv[i]);
                                } else
                                    showHelp("exepected paramter: timout (s)",EXIT_FAILURE,arg);
                                break;
                            default:
                                showHelp("unknown parameter",EXIT_FAILURE,arg);
                                break;
                ...
                case 'h': // help
                    switch(strlen(arg)) {
                        ...
                        case 3:
                            if (arg[2]=='d') {
                                if (i+1>=argc) showHelp("highest belief max distance requires pararameter");
                                QString numStr(argv[++i]);
                                bool ok = false;
                                params.highBeliefFollowerHighestDist = numStr.toInt(&ok);
                                if (!ok) showHelp("highest belief max. dist. is not an integer");
                                break;
                            }
                    ...
                    if (strlen(arg)==2) { //server: "-s"
                        ...
                        switch (arg[2]) {
                            ...
                            case 'K':
                                ...
                                } else if (strlen(arg)>3 && arg[3]=='r') {
                                    params.solverType=SeekerHSParams::SOLVER_FILTER_KALMAN_REQ_VISIB;
                                } else {
                                    showHelp("unknown Kalman Filter solver type", EXIT_FAILURE, arg);
                                }
                                break;
                            case 'P':
                                if (strlen(arg)==3) {
                                    params.solverType=SeekerHSParams::SOLVER_FILTER_PARTICLE;
                                } else if (strlen(arg)>3) {
                                    switch(arg[3]) {
                                        case 'r':
                                            params.solverType=SeekerHSParams::SOLVER_FILTER_PARTICLE_REQ_VISIB;
                                            break;
                                        case 'u':
                                            params.solverType=SeekerHSParams::SOLVER_FILTER_PARTICLE_ALWAYS_UPDATES;
                                            break;
                                        case 'H':
                                            params.solverType=SeekerHSParams::SOLVER_FILTER_PARTICLE_ALWAYS_UPDATES_HB;
                                            break;
                                        case 'M':
                                            params.solverType=SeekerHSParams::SOLVER_MULTI_FILTER_ALWAYS_UPDATES_HB_PARTICLE;
                                            break;
                                        default:
                                            showHelp("unknown Particle Filter solver type", EXIT_FAILURE, arg);
                                    }
                                }
                                break;
                            default:
                    ...
                    switch(strlen(arg)) {
                       ...
                       case 4:
                        if (arg[2]=='o') {
                            switch(arg[3]) {
                                case 'x':
                                    params.multiSeekerObsCombType = SeekerHSParams::MULTI_SEEKER_OBS_COMB_MAX;
                                    break;
                                case 'n':
                                    params.multiSeekerObsCombType = SeekerHSParams::MULTI_SEEKER_OBS_COMB_MIN;
                                    break;
                                case 'a':
                                    params.multiSeekerObsCombType = SeekerHSParams::MULTI_SEEKER_OBS_COMB_AVG;
                                    break;
                                default:
                                    showHelp("unknown parameter", EXIT_FAILURE, arg);
                                    break;
                ...
                case 'n':
                    if (strlen(arg)!=3) {  //expecting "-n?"
                        ...
                        case 'd': {
                            params.useDynObstForVisibCheck = false;
                            break;
                        }
                        case 'r': {
                ...
                case 'e': {
                    if (strlen(arg)==2) {
                        ...
                        switch(arg[2]) {
                            case 'w': {
                                //AG160725: added ext. score
                                if (i+4>=argc) showHelp("exploration weight requires 4 pararameter: util, dist, belief weight, external score",EXIT_FAILURE);
                                bool ok=false;
                                QString dStr(argv[++i]);
                                ...
                                //AG160725: ext. score
                                dStr = QString::fromLatin1(argv[++i]);
                                params.multiSeekerExplorerExtScoreWeight = dStr.toDouble(&ok);
                                if (!ok) showHelp("exploration ext. score weight expected");
                                break;
                            }
                            case 'm': {
                            ...
                            case 'M': {
                                //AG160725: added ext. score
                                if (i+2>=argc) showHelp("exploration external score expected",EXIT_FAILURE);
                                params.multiSeekerExplorerExtScoreMatFile = argv[++i];
                                bool ok=false;
                                QString dStr(argv[++i]);
                                if (dStr.compare("1")==0 || dStr.compare("true", Qt::CaseInsensitive)==0)
                                    params.multiSeekerExplorerExtScoreNormWMaxDist = true;
                                else if (dStr.compare("0")==0 || dStr.compare("false", Qt::CaseInsensitive)==0)
                                    params.multiSeekerExplorerExtScoreNormWMaxDist = false;
                                else
                                    showHelp("expected true/1/false/0 as second parameter of -eM to indicate if we should normalize by max. distance");
                                break;
                            }
                            default:
                ...
                case 'E': {
                    if (strlen(arg)==2) {
                        if (i+1>=argc) showHelp("expected experiment name");
                        params.expName = argv[++i];
                    } else if (strlen(arg)!=3) {
                            ...
                            case 's': // standard deviation of noise
                                switch (arg[3]) {
                                    ...
                                    case 'h': // heading
                                        switch (arg[4]) {
                                            case 'h': // hider
                                                params.contHiderHeadStdDev = stdDev;
                                                break;
                                            default:
                                                showHelp("unknown parameter / std.dev. type", EXIT_FAILURE,arg);
                                                break;
        ...
        */
        ...
        if (runMode==MODE_RUN_GAME || runMode==MODE_TEST) {
            for(size_t i=0; i<params.numPlayersInClient; i++) {
                seekerHSVec[i] = new SeekerHS(&params, runMode==MODE_TEST, i);
            }
        }
        ...
        switch (runMode) {
            case MODE_RUN_GAME: {
                ...
                //AG160203: if using max time out, show it:
                if (checkConnWaitTime_msec>0)
                    cout << "Max. wait for server time-out: "<<checkConnWaitTime_msec/1000.0<<" s"<<endl;
                ...
                //AG160202: check connection, and if it takes too long, exit
                CheckConnection checkConn(&gameConnector);

                if (checkConnWaitTime_msec>0) {
                    checkConn.start(checkConnWaitTime_msec);
                }
...
void showHelp(string error, int exitCode, char* arg) {
    ...
    cout << "hsmomdp [-t | -ts | -tm | -tc | -tC | -tg | -tp | -h | -v] [ -Gh | -Gf[2|m] ] [-so | -sl | -slc | -sm | -ss | -s[2|M][c|d] | -sc[c|d|p] |"
         << "-sf[e|a] | -sC[c|h|H] | -sh | sK[r] | -sP[r|u|H|M] ] [-Rf | -Rc | -Rt | -Rff | -Rfd ] [-D discount] [-gt | -gb | -gk [k] | -g[r|c][x] "
         ...
         << "| [-fd follow-dist] | [-nc] | [-ld] | -nr num-players-req | -np num-players-this-client | [-dv] | [-to time-out-server-s] | [-nd] | [-hd max-dist]"
         << "[-mo[n/x/a]] " << endl
         ...
         << "   -sP[r|u|H|M]      solver: Particle Filter, r: requires an observation for each step, u: always update (also if no obs.),"<<endl
         << "                      H: like u, and uses Highest Belief, M: multi robot, with HB explorer"<<endl
         ...
         << "   -E exp-name       experiment name (can be of experiment group)" << endl
         ...
         << "   -n[i|b|P] num     number of init belief states (or particles), if 0 (default) all belief points of init will be used"<<endl
         ...
         << "   -cshh             std_dev continuous std. dev. for heading hider (default = "<<SeekerHSParams::CONT_HIDER_HEAD_STDDEV<<")"<<endl
         ...
         << "   -hd max-dist      max. distance until which highest belief is search for"<<endl
         ...
         << "   -pph torizon    time-horizon for people predictor"<<endl
         << "   -bi belief-image  image file of belief (png, default: out.png)"<<endl
         << "   -ew uw dw bw ew   set weights for explorer formula: util, distance, belief weight, external score weight"<<endl
         ...
         << "   -dv               disable visibility check for simulation based on distance (POMCP)"<<endl
         << "   -to time-out-s    time out after which no answer has been received of the server, exits with code 10"<<endl
         << "   -nd               do not use dynamic obstacles"<<endl
         << "   -eM mat-file nrmW load score matrix file for explorer with normalization (nrmW=1) or without (nrmW=0)"<<endl
         << "   -mo[n/x/a]        multi player combine observation (weights) using min/max/average(default)" <<endl
         << endl;
...
void showVersion() {
    ...
    #endif
    cout << endl;
    cout << "Random (1-10): "<<hsutils::random(1,10)<<endl;
