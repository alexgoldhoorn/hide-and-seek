...
TwoSeekerHBExplorer::TwoSeekerHBExplorer(SeekerHSParams* params, AutoPlayer* autoPlayerWBelief) :
    HighestBeliefFollower(params,autoPlayerWBelief), _otherSeekerPlayer(nullptr) //, _posScoreMat(nullptr)
{
    ...
    //AG160727: load external matrix file
    if (_params->multiSeekerExplorerExtScoreMatFile.empty()) {
       _posScoreMat = nullptr;
    } else {
        _posScoreMat = new PosScoreMat(_params->multiSeekerExplorerExtScoreMatFile);

        if (_params->multiSeekerExplorerExtScoreWeight<=0) {
            cout << "TwoSeekerHBExplorer::TwoSeekerHBExplorer: WARNING the external score weigth <= 0!"<<endl;
        }
    }
}
...
bool TwoSeekerHBExplorer::initBeliefRun() {
    ...
    //AG160728: load file, now map is loaded
    if (_posScoreMat!=nullptr) {
        _posScoreMat->loadScores(_map->rowCount(), _map->colCount());
    }
...
void TwoSeekerHBExplorer::fixSeekersNotToBeClose(const Pos& nextPos1In, const Pos& nextPos2In, Pos& nextPos1Out, Pos& nextPos2Out) {
    ...
    if (_map->isVisible(nextPos1In, nextPos2In,false,false)) { //AG150917: bug fix: first param was also nextPos2In -> was always true
...
int TwoSeekerHBExplorer::calcExplorationEvaluation(const Pos &seekerPos, const std::vector<BPos> &highestBeliefPosVec,
    ...
    DEBUG_CLIENT(cout<<"TwoSeekerHBExplorer::calcExplorationEvaluation: for seeker "<<seekerPos.toString()<<":"<<endl;);
    ...
    if (highestBeliefPosVec.size()==1) {
        ...
        double maxDist = _map->distance(highestBeliefPosVec[0 /*MULTI_SEEKER_VEC_OWN_INDEX*/], seekerPos);
        double maxBelief = highestBeliefPosVec[0 /*MULTI_SEEKER_VEC_OWN_INDEX*/].b;
        for(size_t i = 1; i < highestBeliefPosVec.size(); i++) {
            if (highestBeliefPosVec[i].isSet()) {
                ...
                if (d > maxDist)
                    maxDist = d;
                //check highest belief  //AG160217: bug fix, maxBelief was maxDist  !!
                if (highestBeliefPosVec[i].b > maxBelief)
                    maxBelief = highestBeliefPosVec[i].b;
            }
        }
        ...
        for(size_t i=0; i < highestBeliefPosVec.size(); ++i) {
            if (highestBeliefPosVec[i].isSet()) {
                ...
                //AG160725
                double extScore = 0;
                if (_posScoreMat != nullptr && _params->multiSeekerExplorerExtScoreWeight>0) {
                    //get external score
                    extScore = _posScoreMat->getScore(highestBeliefPosVec[i].row(),highestBeliefPosVec[i].col());
                    if (_params->multiSeekerExplorerExtScoreNormWMaxDist) {
                        //normalize with distance
                        extScore /= maxDist;
                    }
                    //add to score
                    y += _params->multiSeekerExplorerExtScoreWeight * extScore;
                }
                ...
                DEBUG_CLIENT(cout<<"  "<<i<<") pos="<<highestBeliefPosVec[i].toString()<<",u="<<uVec[i]<<",d="<<d<<",e="<<extScore<<", y="<<y<<endl;)
            }
        }
    ...
    if (updateU) {
        ...
        for(size_t i=0; i < highestBeliefPosVec.size(); ++i) {
            ...
            if (d < _params->multiSeekerExplorerMaxRange) {
                uVec[i] -= (1.0 - d/_params->multiSeekerExplorerMaxRange);
            }
        }
    }
...
void TwoSeekerHBExplorer::setScoreMat(PosScoreMat *scoreMat) {
    _posScoreMat = scoreMat;
}
