...
HSServer::HSServer(QString xmlConfigFile) {
    ...
    cout << "Simulating empty observation if too far: ";
    if (_config->simNotVisibDist()) {
        cout << "yes"<<endl; // (x0="<<_config->_simNotVisibDist_x0<<", k0="<<_config->_simNotVisibDist_k0<<", y0="<<_config->_simNotVisibDist_y0<<")"<<endl;
    } else {
        cout << "no"<<endl;
    }
    ...
    if (!_hstcpserver->listen(QHostAddress::Any,port)) {
        ...
        QCoreApplication::exit(-1); //to be sure
    }
