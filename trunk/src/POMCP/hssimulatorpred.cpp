...
void HSSimulatorPred::resetParams() {
    ...
    covarVec[0] = covarVec[3] = hsutils::sqrd( _params->contNextHiderStateStdDev );
    covarVec[1] = covarVec[2] = 0;
    _nextHiderStepDistr.setCovar(covarVec);

    //set covar next seeker step
    //vector<double> covarVec(4);
    covarVec[0] = covarVec[3] = hsutils::sqrd( _params->contNextSeekerStateStdDev );
    //covarVec[1] = covarVec[2] = 0;
    _nextSeekerStepDistr.setCovar(covarVec);

    //set covar hider obs
    covarVec[0] = covarVec[3] = hsutils::sqrd( _params->contHiderObsStdDev );
    _hiderObsDistr.setCovar(covarVec);

    //set covar seeker obs
    covarVec[0] = covarVec[3] = hsutils::sqrd( _params->contSeekerObsStdDev );
    _hiderObsDistr.setCovar(covarVec);
...
void HSSimulatorPred::getRandomPersonPoint(const double &r_in, const double &c_in, double &r_out, double &c_out) {
    ...
    if (predAvail && _uniformProbDistr(_randomGenerator) <= _params->contUseHiderPredStepProb) {
        ...
        do {
            ...
            //TODO: AG160505: use dyn. obst. in player
        } while (!_map->isPosInMap(r_out,c_out) || !_map->isVisible(r_in, c_in, r_out, c_out, false,_params->simNotVisibDist)); //AG160216: TODO check if using max dist
    }
