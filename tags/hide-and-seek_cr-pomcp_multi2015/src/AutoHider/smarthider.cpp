...
Pos SmartHider::getNextPosRun(int actionDone, int *newAction) {
    ...
    if (opponentVisible) {
        ...
        if (opponentVisible)
            ...
            opponentVisible = _map->isVisible(seekerPos,hiderPos,true);
    }
    ...
    for(int a=0;a<HSGlobalData::NUM_ACTIONS;a++) {
        double s=0;
        bool actOK = scoreForAction(seekerPos,hiderPos,a,s); //seekerPos.row(),seekerPos.col(),hiderPos.row(),hiderPos.col(),a,opponentVisible,s);
        if (actOK && s>=maxS) {
            //maxA = a;
            if (s>maxS) {
                //it's better
...
bool SmartHider::scoreForAction(const Pos& seekerPos, const Pos& hiderPos /*int sr, int sc, int hr, int hc, int a, bool visib*/,  int a, double& s) {
    //assert(!isSeeker());

    /*_hiderPlayer->*/ //playerInfo.currentPos.set(hr,hc);

    //DEBUG_AUTOHIDER(cout<<" scoreForAction: "<<ACTION_COUT(a)<<" from "<</*_hiderPlayer->*/playerInfo.currentPos.toString()<<":"<<flush;);
    ...
    Pos newHPos = _map->tryMove(a, /*_hiderPlayer->playerInfo.currentPos*/ hiderPos);
    if (newHPos.isSet()) {

        DEBUG_AUTOHIDER(cout <<"ok: "<<newHPos.toString()<<endl;);

        s = score(seekerPos, hiderPos); // sr,sc,newHPos.row(),newHPos.col(),visib); //score(sr,sc,newHPos.row(),newHPos.col(),visib);
...
double SmartHider::score(const Pos& seekerPos, const Pos& hiderPos) { //int sr, int sc, int hr, int hc, bool visib) {
