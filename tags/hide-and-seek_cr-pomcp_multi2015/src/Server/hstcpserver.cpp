...
void HSTCPServer::handleNewConnection() {
    ...
    if(socket->isOpen()) {
        ...
        if (_numConn >= HSGlobalData::MAX_NUM_PLAYERS || (_params!=NULL && _numConn>=_params->maxNumberOfPlayers())) { //AG150430: use num of players
                                        //_numConn > 2 || (_params!=NULL && !_params->gameHas2Seekers() && _numConn > 1)) {
        ...
        case HSGlobalData::P_Seeker: // PLAYER_TYPE_SEEKER:
            player->playerType=HSGlobalData::P_Seeker;
            if (_seeker1Player==NULL) {
                _seeker1Player = player;
            }
            break;
        case HSGlobalData::P_Hider: //PLAYER_TYPE_HIDER:
...
bool HSTCPServer::readNewAction(QDataStream& in, PlayerInfoServer *player /*, double* beliefScore, double* seekerReward*/) {
    assert(player!=NULL);
    //retrieve info
    ...
    /*DEBUG_SERVER(cout COUT_SHOW_ID << player->toString() <<" Server reads: "<<player->numberActions +1
                                    <<" action="<<ACTION_COUT((int)action)<<", "<<pos.toString(););*/

    //AG140409: belief score (NOTE: this is of the previous position and belief)
    //double beliefScoreRec;
    in >> player->seekerBeliefScore; // beliefScoreRec;
    /*if (beliefScore!=NULL) {
        *beliefScore = beliefScoreRec;
    }*/
    //AG140612: seeker reward
    //double reward;
    in >> player->seekerReward; // reward;
    /*if (seekerReward!=NULL) {
        *seekerReward = reward;
    }*/

    //AG150216: read multi chosen pos
    //if (_params->seekerSendsMultiplePoses() /*gameHas2Seekers()*/ && player->isSeeker()) {
        //AG150717: always read chosen goal
...
void HSTCPServer::writeUpdateToStream(QDataStream &out, QByteArray& byteArray, /*ClientSocket* client,*/ /*const Pos& pos,*/
    ...
    cout << "{i"<<_playerVec[0]->numberActions<<"}";
    ...
    for(PlayerInfoServer* player : _playerVec) {
        ...
        DEBUG_SERVER_VERB(cout COUT_SHOW_IDcout <<"- "<<player->id<<" curpos: "<<player->currentPos.toString()<<" w noise:"<<player->currentPosWNoise.toString()<<endl;);
    }

    //AG150504: for each not seeker player send a hider noisy pos:
    //out << (quint8)client->getNumSeekers(); //num players to send hider pos for



        //TODO:  generate the observations ONLY ONCE! and not for each client!
        //       be sure the vision test is wrt the specific player/seeker
        //
    ...
    //    PlayerInfoServer* player = client->getPlayer(i);
    for(PlayerInfoServer* player : _playerVec) { //AG150711: all players should be sent
        if (player->isSeeker()) {
            ...
            out << player->useObsProb; //(player->hiderObsTrustProb<0 ? player->hiderObsTrustProbFixed : player->hiderObsTrustProb);
            DEBUG_SERVER_VERB(cout COUT_SHOW_IDcout <<"- "<<player->id<<" hiderobs: "<<player->hiderObsPosWNoise.toString()<<endl;);
...
void HSTCPServer::sendUpdateToPlayers() {
    ...
    // TODO!! adapt DB functions
    long logLineID = writeStepToLog(); // &hiderPosWNoise, hiderPosWNoise2P); //log the actions in the files.
    if (logLineID <0)
        cout << "HSTCPServer::sendUpdateToPlayers - WARNING: logging failed (id="<<logLineID<<")"<<endl;
}

Pos HSTCPServer::applyNoiseHider(Pos seekerPos, Pos hiderPos) {
...
void HSTCPServer::agentReadyRead(ClientSocket* client) { //, PlayerInfo &playerInfo, double *seekerBeliefScore, double* seekerReward) {
    ...
    switch ((MessageType)messageType) {
        case MT_InitPos: {
            ...
            if (_numPlayersWInitPos == _numPlayers) { //_hiderPlayer.set && _seeker.set && (!_params->gameHas2Seekers() || _seeker2.set)) {
        ...
        case MT_Action: {
            ...
            bool someHadFlag = readNewActions(in); //, &playerInfo, seekerBeliefScore, seekerReward);

            if (someHadFlag) {
                cout COUT_SHOW_ID <<" HSTCPServer::agentReadyRead: there is a client which already had updated it's location, ignoring"<<endl;
            }
        }
        break;
        case MT_SeekerGoals:
...
int HSTCPServer:: checkStatus() {
    ...
    //bool use2Robots = (_params->gameHas2Seekers());
    //check if using multiple robots
    //bool useMultiRobot = _params->seekerSendsMultiplePoses();
    ...
    if (_params->stopAtWin) {
        ...
        if (_params->gameType==HSGlobalData::GAME_HIDE_AND_SEEK) { //AG150511: limit to hide-and-seek only
            ...
            if (_config->getWinIfCrossed()) {
                ...
                /*if (use2Robots && !winSeeker)
                    winSeeker = _seeker2.currentPos.equalsInt(_hiderPlayer.previousPos)
                            && _seeker2.previousPos.equalsInt(_hiderPlayer.currentPos); // check crossed for 2nd seeker
                            */
            }
            if (!winSeeker) {
                if (_params->useContinuousPos) {
                    ...
                    //AG150112: check for 2 robots
                    /*if (use2Robots && !winSeeker)
                        winSeeker = (_gmap->distanceEuc(_seeker2.currentPos, _hiderPlayer.currentPos)<=_params->winDist);*/
                } else if (_params->winDist==0) {
                    winSeeker = seekerPlayer->currentPos.equalsInt(_hiderPlayer->currentPos);

                    //AG150112: check for 2 robots
                    /*if (use2Robots && !winSeeker)
                        winSeeker = _seeker2.currentPos.equalsInt(_hiderPlayer.currentPos);*/
                } else {
                   winSeeker = _gmap->distance(seekerPlayer->currentPos,_hiderPlayer->currentPos)<=_params->winDist;

                   //AG150112: check for 2 robots
                   /*if (use2Robots && !winSeeker)
                       winSeeker = _gmap->distance(_seeker2.currentPos,_hiderPlayer.currentPos)<=_params->winDist;*/
                }
            }
        }
        ...
        if (winSeeker) {
            ...
            if (!winHider) {
                if (_params->maxNumActions>0 && (/*_seeker.numberActions >= _params->maxNumActions ||*/
                                                 _hiderPlayer->numberActions >= _params->maxNumActions)) { //TODO: put a max time
...
void HSTCPServer::initializeLogs() { //TODO: UPDATE
    //AG150118: if no log
...
long HSTCPServer::writeStepToLog() { //(const Pos* hiderPosWNoiseSent, const Pos* hiderPosWNoiseSent2) {
...
void HSTCPServer::writeGameParamsToStream(QDataStream &out, QByteArray &byteArray, ClientSocket* client) { //PlayerInfo* recPlayer) { //, char playerType, PlayerInfo *p2Info,
