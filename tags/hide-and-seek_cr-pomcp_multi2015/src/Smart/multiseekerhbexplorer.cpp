...
bool MultiSeekerHBExplorer::calcNextHBList(int actionDone) {
    ...
    //playerInfo.multiHBBeliefVec.clear();
...
Pos MultiSeekerHBExplorer::selectRobotPosMulti() {
    ...
    if ( _chosenHiderPos.isSet() || //AG15022: if hider pos known, always update goal
        ...
        sort(_piWrapperVec.begin(), _piWrapperVec.end(), greater<PlayerInfoWrapper>());
...
void MultiSeekerHBExplorer::combineHBPoses(std::vector<BPos> &hbPosesVec) {
    ...
    //copy all hb points from own player
    //hbPosesVec.insert(hbPosesVec.end(), playerInfo.multiHBPosVec.begin(), playerInfo.multiHBPosVec.end());
    //AG150717: copy this way, since insert copies by pointer, and we want to change the values
    /*for(BPos bpos : playerInfo.multiHBPosVec) {
...
void MultiSeekerHBExplorer::useFollower(const Pos& hiderPos) {
    ...
    if (_params->minDistBetweenRobots>0) {
        ...
        //_otherSeekerPlayer->chosenGoalPos = hiderPos;
