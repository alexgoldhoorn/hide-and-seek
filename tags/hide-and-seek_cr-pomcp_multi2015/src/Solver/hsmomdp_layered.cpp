...
LayeredHSMOMDP::LayeredHSMOMDP(SeekerHSParams* params) : HSMOMDP(params) {
    _topSolver = NULL;
...
void LayeredHSMOMDP::setSegmenter(Segmenter* segmenter) {
    _segmenter = segmenter;
}

Segmenter* LayeredHSMOMDP::getSegmenter() const {
    return _segmenter;
}

void LayeredHSMOMDP::setSegmenterX(Segmenter* segmenter) {
    _segmenterX = segmenter;
}

Segmenter* LayeredHSMOMDP::getSegmenterX() const {
    return _segmenterX;
}
...
bool LayeredHSMOMDP::initBelief(GMap* gmap, Pos seekerInitPos, Pos hiderInitPos, bool hiderVisible) {
    ...
    //get state of base
    _baseState = _map->getIndexFromCoord(_map->getBase());
...
bool LayeredHSMOMDP::updateBelief(int observ, int x_state) {
    ...
    //AG120829: init position also in segmenter
    Pos seekerPos = _map->getCoordFromIndex(x_state); //ag130304: was stateToPos(x_state);
...
bool LayeredHSMOMDP::isFinalState(unsigned int x, unsigned int y, vector<int>& xBs, vector<int>& yBs, bool& winSeeker) {
    ...
    if (_segmentSeekerStates) {
        ...
        for(unsigned int yc=0; yc<yBs.size() && !winSeeker; yc++) {
            if ((int)x==yBs[yc]) { //TODO: later use distance to check if final state...
                winSeeker = true;
            } else if (yBs[yc]==_baseState) {
                winHider = true;
            }
...
bool LayeredHSMOMDP::checkTopMOMDP() {
    ...
    return true; //TODO: conclude from results
}
...
int LayeredHSMOMDP::getBestAction() {
    ...
    double reward = 0,bottomReward = 0;//ag120904: reward
    int bottomAction = -1, rankTopInBottom = -1, rankBottomInTop = -1;
...
int LayeredHSMOMDP::getRankOfAction(int a, vector<double>* valPerAction) {
    ...
    int rank = 1;
    //cout << "[";
    for(size_t i=0; i<valPerA.size(); i++) {
        //get value
        double v = (valPerA)[i];
        //cout << v <<",";
...
int LayeredHSMOMDP::getNextAction(int observ, int x_state) {
    ...
    if (_action!=HSGlobalData::ACT_H|| _segmenter->segmentsCanChangeWhenSeekerHalt()) {
        ...
        showMOMDP(_momdpTop, DEBUG_SHOW_TRANS);
    }
...
int LayeredHSMOMDP::getNextAction(Pos seekerPos, Pos hiderPos, bool visible, int actionDone) {
    ...
    return HSMOMDP::getNextAction(seekerPos, hiderPos, visible, actionDone);
}


vector<int> LayeredHSMOMDP::getNextMultipleActions(Pos seekerPos, Pos hiderPos, bool opponentVisible, int actionDone, int n) {
    throw CException(_HERE_, "LayeredHSMOMDP::getNextMultipleActions: not yet implemented");
}


std::string LayeredHSMOMDP::getName() const {
    return "HSMOMDP_layered";
}
