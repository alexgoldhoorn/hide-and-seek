...
void Solver::solve(Model& currModel, vector<MState> &initialBeliefStates, vector<long> &pathLength)
{
    MObs obs(vector<long>(currModel.getNumObsVar(),0));
    obs.obs[1] = -1;

    this->solve(currModel, initialBeliefStates, obs, pathLength);
}

void Solver::solve(Model& currModel, vector<MState> &initialBeliefStates, MObs& obs, vector<long> &pathLength)
{
...
void Solver::solve(Model& currModel, MState& initialBeliefState, long pathLength)
{
    MObs obs(vector<long>(currModel.getNumObsVar(),0));
    obs.obs[1] = -1;
...
void Solver::solve(Model& currModel, MState& initialBeliefState, MObs& obs, long pathLength)
{
    MAction::initStatic(&currModel);
