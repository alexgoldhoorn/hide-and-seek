...
#include "Model.h"
#include "MBelief.h"
#include "MBeliefNode.h"
#include "MBeliefSet.h"
...
void BeliefTree::expandNodes(double target)
{
    ...
    if (debug) {
        ...
        map<MObs,ObsEdge>::iterator it = currNode->beliefNode->actNodes[0]->obsChildren.begin();
        cout<<"cachedParticles = NULL? "<<((it->second).cachedParticles == NULL)<<endl;
    }

    map<MObs,ObsEdge>::iterator iter = findBestObs(currNode, currTarget, excessUncertainty);

    while (excessUncertainty > 0){
        MAction* act = &(currNode->beliefNode->bestUBoundAct);
        assert(act->actNum != -1);
...
inline map<MObs,ObsEdge>::iterator BeliefTree::findBestObs(Belief *currNode, double target, double& excessUncertainty)
{
    bool debug = false;
    ...
    map<MObs,ObsEdge>::iterator iter = currNode->beliefNode->actNodes[actIndex]->obsChildren.begin();
    double currBest = NegInf;
    map<MObs,ObsEdge>::iterator currBestObs = currNode->beliefNode->actNodes[actIndex]->obsChildren.end();
