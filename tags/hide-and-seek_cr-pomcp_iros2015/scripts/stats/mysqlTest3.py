...
"""select concat_ws(',',MapID,HiderCat3,SeekerType2,Reward ), sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total
from <TABLE>
where MapSize<>'40x40' and HasTimeLimit='no'
group by MapID,HiderCat3,SeekerType2,Reward
order by MapID,HiderCat3,SeekerType2,Reward"""]
...
"""select concat_ws(',',OnOffline, SeekerType2, Reward, MapID, IsWin), <COL>
from <TABLE>
where MapSize<>'40x40'"""]
]
...
qry_mapidsize_summ = """select concat_ws(',',MapSize,SeekerType2,Reward) as N, 
sum(IsWin) as Win, sum(IsLose) as Lose, sum(IsTie) as Tie, count(*) as Total,
NumActions_avg,NumActions_std,DurationPerAction_avg,DurationPerAction_std
from <TABLE> as s1 left join
(select concat_ws(',',MapSize,SeekerType2,Reward) as N2, 
avg(NumActions) as NumActions_avg, std(NumActions) as NumActions_std,
 avg(DurationPerAction) as DurationPerAction_avg, std(DurationPerAction) as DurationPerAction_std
from <TABLE>
where HasTimeLimit='no' and IsWin=1
group by MapSize,SeekerType2,Reward
) as s2 on concat_ws(',',MapSize,SeekerType2,Reward) =s2.N2
where HasTimeLimit='no'
group by MapSize,SeekerType2,Reward
order by MapSize,SeekerType2,Reward"""
