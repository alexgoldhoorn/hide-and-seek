...
def runPomcp(mapname,reward,depth,numSim,numBel,explCount,expandCount,expectedRewCount,hiderSim,hiderSimRandomP):	
	...
	if (doRun==True):
		cmd = "./hsmomdp -o%s %s -sc -u %s -m %s -R%s -d %d -ns %d -ni %d -x %d -e %d -E%s -hs%s %f > expbatch_pomcp.log" % (opponent,mapactfile,user,mapname, reward, depth, numSim, numBel, explCount, expandCount, expectedRewCount, hiderSim, hiderSimRandomP, params) # ,logfile)
		print cmd
		subprocess.call(cmd, shell=True)
...
def doMap(mapname,size):
	...
	expandC = [2,10]
	explorC_triangle = [ceil(0.5*rewardDiff),rewardDiff,ceil(1.5*rewardDiff)]
	explorC_simple = [1,2,10]
	hiderSims = ['b','r']
	hiderSimRandProb = [0,0.1,0.5]
	# 3x1x3x2x3	
