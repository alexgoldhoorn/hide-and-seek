...
def runPomcp(mapname,reward,depth,numSim,numBel,explCount,expandCount):	
	...
	if (doRun==True):
		cmd = "./hsmomdp -o%s %s -sc -u %s -m %s -R%s -d %d -ns %d -ni %d -x %d -e %d %s > /dev/null" % (opponent,mapactfile,user,mapname, reward, depth, numSim, numBel, explCount, expandCount, params) # ,logfile)
		print cmd
		subprocess.call(cmd, shell=True)
...
def doMap(mapname,size):
	...
	#params
	depth = [5,ceil(0.5*maxAct),maxAct]
	numSim = [100,1000] #,10000]
	numBel = [ceil(0.05*cells),ceil(0.1*cells)]
	expandC = [2,10,20]
	explorC_triangle = 3*cells
	explorC_simple = 2
	...
	for d in depth:
		for ns in numSim:
			for ni in numBel:
				for e in expandC:
					runPomcp(mapname,'t',d,ns,ni,explorC_triangle,e)
					runPomcp(mapname,'f',d,ns,ni,explorC_simple,e)
