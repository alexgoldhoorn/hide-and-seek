...
def runSmart(mapname):
	pomdpfile = "%s.pomdpx" % (mapname)
#	polfile = "%s/%s_%s.pol" % (poldir,mapname,poltyp)
	mapactfile = ""
#	sarsoplogfile = "SARSOP_time_log_%s_%s_%s.txt" % (mapname,typ2,exp)
	user = "SmartSeeker" # (mapname,typ2,exp)
	logfile = "%s_%s.log" % (user,trial)
...
#	print "SARSOP log:		"+sarsoplogfile
	...
	if (doRun==True):
		cmd = "./hsmomdp -o%s %s -ss -u %s -m %s %s > %s" % (opponent,mapactfile,user,pomdpfile,params,logfile)
		print cmd
		subprocess.call(cmd, shell=True)
