...
MState ParticlesBelief::average() const
{
    MState avg(beliefNode->model->getNumStateVar(),0);
...
Belief* ParticlesBelief::beliefFromState(const MState& st, const MObs& obs, long pathLength)
{
...
Belief* ParticlesBelief::beliefFromStateSet(const vector<MState>& st, const MObs& obs, const vector<long>& pathLength)
{
    ParticlesBelief *pb = new ParticlesBelief(new BeliefNode(obs));
...
*/
Belief* ParticlesBelief::nextBelief(const MAction& action, const MObs& obs) const
{
    bool debug = false;
    ...
    for (long i = 0; i < numRandStreams; ++i) {
        ...
        MState nextState(beliefNode->model->getNumStateVar(),0);
        MObs currObs(vector<long>(beliefNode->model->getNumObsVar(),0));
