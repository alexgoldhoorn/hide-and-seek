...
/*! This class handles the game connection to the server and the game flow.
  ...
  - refactor!!
  */
class GameConnector : public Game
{
    Q_OBJECT
public:
    ...
    GameConnector(string ip, int port, int type, AutoPlayer* autoplayer, SeekerHS* seekerHS = NULL, int mapID = 0,
                  int optype=0, string username="SeekroBot", int numofgames=1, string actionFileList="", int winDist=0, GMap* mapToSend=NULL);
    ...
    //AG131007: start connecting and running
...
private:
    ...
    //void init(GMap* gmap); //ag131007: disabled, not used

    //ag131007: int -> start and removed params
...
#ifndef SEEKERHS_CHECKS
    unsigned int _actionsDone;
#endif
