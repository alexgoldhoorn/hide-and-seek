...
GameConnector::GameConnector(string ip, int port, int type, AutoPlayer* autoplayer, SeekerHS* seekerHS, int mapID, int optype,
                             string username, int numofgames, string actionFileList, int winDist, GMap* mapToSend) {
        _port= port;
        ...
        //AG131007: updated to start method
...
void GameConnector::startGame() { //(int type,  Player* pl, AutoPlayer* autoplayer, string username, GMap* gmap){
    //if (gmap==NULL) {
        _gmap = _player->getMap(); //use this map,.. it will write it from server even if it's already set..
    //} else {
    //    _gmap = gmap;
    //}
    ///_player = pl;
    ...
    if(_player->getType()==1) {
        ...
        DEBUG_CONNECTOR(cout<<"My name is "<<_username<<endl;);
    ...
    if(tcpSocket.waitForConnected(3000))
    {
        ...
        in >> rows;
        in >> cols;
        DEBUG_CONNECTOR(cout << "map size: "<<rows<<"x"<<cols<<endl;);
        _gmap->createMap(rows,cols);

        quint32 obstCount;
        in >> obstCount; // num of obstacles
        unsigned int count;
        DEBUG_CONNECTOR(cout<<"rh: Obstacles : "<<obstCount<<endl;);
        ...
        DEBUG_CONNECTOR(cout<<"rh: "<<"Base : "<<_gmap->getBase().row<<"/"<<_gmap->getBase().col<<endl;);
        ...
        DEBUG_CONNECTOR(cout<<"rh: "<<"my pos : "<<(int)x<<"/"<<(int)y<<endl;);
        if (_player->getType())
        ...
        if(!_player->getType()) {//hider
            ...
            _player->setOppUsername(un.toStdString());
        }
        else {//seeker
           //_gmap->setSeeker(playerPos);
           //_player->calculatevisible(); //ag130228: disabled
           DEBUG_CONNECTOR(cout<<"rh: "<<"You are the seeker!"<<endl;);
           _player->setOppUsername(un.toStdString());
        }
        ...
        if(_player->getType()){//if its the seeker
            ...
            if (_seekerHS!=NULL) {
                ...
                _gmap->printMap(playerPos,opPlayerPos);
                ...
                DEBUG_CONNECTOR(cout << " real check"<<endl;);
                ...
                if (checkHiderPos && checkSeekerPos) {
                    DEBUG_CONNECTOR(cout << "[seeker+hider pos checks SUCCEEDED]"<<endl;);
                }

                DEBUG_CONNECTOR(cout << "SEEKERHS_CHECKS for init DONE"<<endl;);

                _seekerHS->init(sPosVec,hPosVec);

                DEBUG_CONNECTOR(cout << "SEEKERHS_INIT DONE"<<endl;);
...
*/
	...
	//AG111202: test if ready for read..
        _stopped = false;
        DEBUG_CONNECTOR(cout << "ready read test..."<<endl;);
        readyReadTest();
        DEBUG_CONNECTOR(cout << "done ready read"<<endl;);
    }
    else
...
void GameConnector::sendInfomsg(GMap* gmap){
    ...
    tcpSocket.write(block);
    tcpSocket.flush();
    DEBUG_CONNECTOR(cout<<"rh: "<<"player sent his name : "<<_player->getUsername()<<endl;);
...
//msg format:  blocksize,  0, curpos(x,y), errorstring
 ...
 //             blocksize, 1, oppos(x,y)
    ...
    DEBUG_CONNECTOR(cout<<"rh:  Inside readyRead()!!!!!"<<endl;);
...
void GameConnector::takenewaction(int a){
    ...
    if(sendaction())
       DEBUG_CONNECTOR(cout<<"rh: action sent"<<endl;);
...
void GameConnector::takenewseekeraction(){
   ...
   /*At this point we have available the observations. (the first time is the initial position of the seeker)
    ...
    Pos hiderPos = _player->getOppPos();
    ...
    if (_seekerHS!=NULL) {
        ...
        cout << "Action "<< _seekerHS->getActionsDone() << "/" << _seekerHS->getMaxNumActions() <<": "<< ACTION_COUT(action)
             << " new pos: ("<<sPosVecNew[0]<<","<<sPosVecNew[1]<<") winState="<< WINSTATE_COUT(winState) <<endl;


    } else {
        action = _autoplayer->getNextAction(seekerPos,hiderPos,visible);

        cout << "Action "<< _seekerHS->getActionsDone() << "/" << _seekerHS->getMaxNumActions() <<": "<< ACTION_COUT(action)<<endl;
    }
    ...
    cout << "Action ";
    if (_seekerHS!=NULL) {
        cout << _actionsDone << "/" << _seekerHS->getMaxNumActions();
    }
    cout <<": "<< ACTION_COUT(action)<<endl;
#endif
...
//msg format: action, current position(row,col)
    ...
    _player->unsetf();
    _player->addpmsg();
    DEBUG_CONNECTOR(cout<<"rh: action sent! "<<_player->getLastAction()<<p.row<<p.col<<endl;);
...
{
    cout<<"Waiting to get opponent's initial position "<<flush;
    ...
    in >> y;
    p.col=(int)y;
    DEBUG_CONNECTOR(cout<<"seeker: my opponent's positions is: "<<p.row<<p.col<<endl;);
    tcpSocket.flush();
...
void GameConnector::moveToThreadVariables(QThread *thread) {
