...
{
    int comp = MAction::compare(node1->action, node2->action);
    if (comp)
        return (comp == 1);
...
PolicyGraph::PolicyGraph(long numInitPolicies, long numObsVar, long numRoots): numInitPolicies(numInitPolicies), numObsVar(numObsVar), numRoots(numRoots)
{
    ...
    for (long i = 0; i < numInitPolicies; i++){
        PolicyGraph::Node *tempNode = new PolicyGraph::Node(MAction(i));
...
PolicyGraph::Node *PolicyGraph::find(const MObs& obs, const std::vector<Edge>& edges)
{
...
void PolicyGraph::read(std::string filename)
{
    ...
    for (long i = 0; i< numNodes; i++){
        PolicyGraph::Node * tempNode = new Node(MAction(-1));
