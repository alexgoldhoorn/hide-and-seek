...
SFMWalkers::SFMWalkers(SeekerHSParams *params, size_t n, std::vector<Pos> goals) :
    AutoHider(params), AutoWalker(n), _goals(goals), _nWalkers(n)
{
    //assert(nWalkers>0);
    //assert(goals.size()>0);
}
...
SFMWalkers::SFMWalkers(SeekerHSParams *params, size_t n) :
    AutoHider(params), AutoWalker(n), _nWalkers(n)
{
    //assert(nWalkers>0);
}
...
bool SFMWalkers::initBelief(GMap *gmap, Pos seekerInitPos, Pos hiderInitPos, bool opponentVisible) {
    initRandomizer();
