...
void showHelp() {
    cout<<"HSAutoHider: error in the arguments while calling automated player, expecting: "<<endl
       ...
       <<"- "<<HSGlobalData::OPPONENT_TYPE_HIDER_FILE           <<": from file (positions, extra parameter: positions file ile required)" <<endl
...
void testPosList(QString mapFile, QString posFile, SeekerHSParams* params) {
    ...
    autoWalker->initBelief(&gmap,p,p,false);
    cout<<"ok"<<endl;
    ...
    cout<<"Reading all poses:"<<endl;
    IDPos idpos(0,0,0);
    autoHider->initBelief(&gmap,p,idpos,false);
    vector<int> actions;
    for(uint i=0; i<numSteps; i++) {
       ...
       if (firstParamStr.compare("-create",Qt::CaseInsensitive)==0 || firstParamStr.compare("-write",Qt::CaseInsensitive)==0) {
           ...
           return 0;
            ...
            if (argc > 4 ) {
                ...
                randomPosDistToB = randPDStr.toInt(&ok);
                if (!ok) {
            ...
            cout << "Automated Hider "<<endl<<endl
                 ...
                 << "Random pos. dist. to b.:" << randomPosDistToB <<endl<<endl;
        }
        ...
        if (randomPosDistToB>0) {
            autoHider->setRandomPosDistToBase(randomPosDistToB);
        }
        ...
        if (writeFile) {
            ...
            cout << "Automated Hider "<<endl<<endl
                 ...
                 << "Continuous movements: "<<(params.useContinuousPos?"yes":"no") <<endl<<endl;

            GMap gmap(mapFile.toStdString(), &params);            
            gmap.printMap();

            //cout << "Create FromListHider: "<<flush;
            //FromListHider flHider(&params, outFile.toStdString());
            //cout <<"ok"<<endl; //<<"Writing file..."<<endl;
