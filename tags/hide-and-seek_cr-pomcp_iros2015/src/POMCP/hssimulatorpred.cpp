...
/*TODO:
 * Implement random movement as in Matlab: choose random direction, .. *
 *
 *
 * */
...
State* HSSimulatorPred::step(const State *state, int action, State*& obsOut, double &reward, bool genOutObs, const State* obsIn,
...
void HSSimulatorPred::resetParams() {
    cout<<"HSSimulatorPred::resetParams()"<<endl;
