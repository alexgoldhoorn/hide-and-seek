...
void SeekerHSParams::printVariables(ostream& stream, bool showExtraParams) {
    ...
    stream    << "Continuous/discrete:    "<<(useContinuousPos?"Continuous":"Discrete")<<endl
              << "Path planner:           ";
    ...
    if (solverType==SOLVER_POMCP || solverType==SOLVER_COMBI_POMCP_FOLLOWER || solverType==SOLVER_COMBI_FOLLOWER_HIGHEST_BELIEF
            || solverType==SOLVER_FOLLOWER_HIGHEST_BELIEF || solverType==SOLVER_MULTI_HB_EXPL) {
        ...
        stream        << "Obs. equals thresh dist:" << obsEqualsThreshDist << endl
                      << "Multi seeker own obs pr:" <<multiSeekerOwnObsChooseProb << endl
    ...
    if (solverType==SOLVER_COMBI_FOLLOWER_HIGHEST_BELIEF || solverType==SOLVER_FOLLOWER_HIGHEST_BELIEF || solverType==SOLVER_MULTI_HB_EXPL) {
...
unsigned int SeekerHSParams::numberOfPlayers() {
    if (gameType==HSGlobalData::GAME_FIND_AND_FOLLOW_2ROB) {
        return 3;
    } else {
        return 2;
    }
