...
void MCTS::init(const  Observation* obs) {//State* initObs, const State* initObs2, double obs1p) {
    ...
    //generate root
    State* initState = _simulator->genInitState(obs); //(initObs, initObs2, obs1p);
    ...
    vector<State*> beliefStateVec = _simulator->genAllInitStates(obs, _params->numBeliefStates); //(initObs,_params->numBeliefStates,initObs2,obs1p);
    ...
    if (_params->numBeliefStates > 0) {
        ...
        if (newBelief->size()<_params->numBeliefStates) {
            ...
            for(size_t i=newBelief->size(); i</*oldBelief->size() */_params->numBeliefStates; i++) {
                ...
                //new state
                State* newState = _simulator->step(state, action, obsOut, rew, false, obs); //(state, action, obsOut, rew, false, obs, obs2, obs1p);
