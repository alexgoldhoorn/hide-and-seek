...
void GMap::printMap(bool showDynObst, int sr,int sc, int hr, int hc, std::ostream& stream) {
    ...
    for (int r=-2; r<=_rows; r++) {
        for (int c=-2; c<=_cols; c++) {
            ...
            //cout << "r="<<r<<"_rows="<<_rows<<endl;
            //cout << "c="<<c<<"_cols="<<_cols<<endl;
            ...
            if (r<0 || r==_rows) {
                ...
                bool isInVis = false;
                //cout << "antes isInVis"<<endl;
                //cout << "sr="<<sr<<"; sc="<<sc<<"; r="<<r<<"; c="<<c<<"; showDynObst="<<showDynObst<<endl;
                if (useVisib) isInVis = !isVisible(sr,sc,r,c,showDynObst); //ag130301: use function, since it generates the visibility map
                
                //cout << "1 antes if"<<endl;
                //cout << "_map[r][c]="<<_map[r][c]<<endl;
            ...
            //cout << "antes ifs"<<endl;
                ...
                case GMAP_FREE_SPACE:
                    //cout << "case GMAP_FREE_SPACE in"<<endl;
                    if (showDynObst) {
                    ...
                    //cout << "case GMAP_FREE_SPACE out"<<endl;
                    break;
                case GMAP_OBSTACLE:
                    //cout << "case GMAP_OBSTACLE in"<<endl;
                    if (isInVis)
                        x='x';
                    else
                        x='X';
                    //cout << "case GMAP_OBSTACLE out"<<endl;
                    break;
                case GMAP_BASE:
                    //cout << "case GMAP_BASE in"<<endl;
                    if (isInVis)
                        x='b';
                    else
                        x='B';
                    //cout << "case GMAP_BASE out"<<endl;
                    break;
                case GMAP_SEEKER:
                    //cout << "case GMAP_SEEKER in"<<endl;
                    if (isInVis)
                        x='s';
                    else
                        x='S';
                    //cout << "case GMAP_SEEKER out"<<endl;
                    break;
                case GMAP_HIDER:
                    //cout << "case GMAP_HIDER in"<<endl;
                    if (isInVis)
                        x='h';
                    else
                        x='H';
                    //cout << "case GMAP_HIDER out"<<endl;
                    break;
          ...
          //cout << "antes:  stream<<x;"<<endl;
            stream<<x;
        //cout << "despues:  stream<<x;"<<endl;
        }
      //cout << "antes: x=='.'"<<endl;
        if (x=='.')
            stream <<"·";         
 ...
 //cout << "printMap out 1"<<endl;
}


void GMap::printMapHz(bool showDynObst, int sr,int sc, int hr, int hc, std::ostream& stream) {
    //AG130228
    bool useVisib = sr>=0;
    ...
    for (int r=-2; r<=_rows; r++) {
        for (int c=-2; c<=_cols; c++) {
            ...
            if (r<0 || r==_rows) {
                ...
                //int id_hidden_zone=(int) gmap_hiden_zones_->getItem2(person_sim_pos_x_gmap,2*person_sim_pos_y_gmap,person_sim_pos_x_gmap,2*person_sim_pos_y_gmap+1); 
                char mapCode = _map[r][c];
...
vector<Pos> GMap::getInvisiblePoints(const std::vector<Pos> &posVecIn, bool checkDynObst) {
    vector<Pos> posVector;
    FORs(r2,_rows) {
        FORs(c2,_cols) {
            if ( _map[r2][c2]!=GMAP_OBSTACLE) {
                ...
                for(const Pos& pos : posVecIn) {
                    if (isVisible(pos.row(),pos.col(),r2,c2,checkDynObst)) {
                        //visible
                        isVisib = true;
                        break;
                    }
...
char GMap::getItem2(int row1, int col1, int row2, int col2) const {
    //std::cout << "Ini getItem2"<<std::endl;
    //std::cout << "_rows="<<_rows<<"; _cols="<<_cols<<std::endl;
    //std::cout << "row1="<<row1<<"; col1="<<col1<<std::endl;
    assert(row1>=0 && row1<_rows);
    assert(row2>=0 && row2<_rows);
    assert(col1>=0 && col1<_cols);
    assert(col2>=0 && col2<_cols);
   
    int uno=((int) _map[row1][col1])*10;
    int dos=(int)_map[row2][col2];
    int to_return=uno+dos;
   // std::cout << "_map[row1][col1]="<<_map[row1][col1]<<std::endl;
   // std::cout << "_map[row2][col2]="<<_map[row2][col2]<<std::endl;

   // std::cout << "uno="<<((int) _map[row1][col1])*10<<std::endl;
   // std::cout << "dos="<<(int)_map[row2][col2]<<std::endl;
   // std::cout << "uno+dos="<<uno+dos<<std::endl;

    char ch_return=(char) to_return;
...
void GMap::setItemHiddenZonesMap(int row, int col, char c) {
    assert(row>=0 && row<_rows);
    assert(col>=0 && col<_cols);
    //assert(c==HiddenZone1 || c==HiddenZone2 || c==HiddenZone3 || c==HiddenZone4 || c==HiddenZone5 || c==HiddenZone6 || c==HiddenZone7 || c==HiddenZone8 || c==HiddenZone9);
