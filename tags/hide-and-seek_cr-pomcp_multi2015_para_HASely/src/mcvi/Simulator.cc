...
void Simulator::runSingle(long length, double& sumReward,
                          ...
                          MState startState, RandStream& randStream,
                          long rootIndex)
{
    ofstream fp;
    ...
    // States
    MState currState = startState; // initialize
    MState nextState(model.getNumStateVar(),0); // allocate space
    ...
    MObs obs(vector<long>(model.getNumObsVar(),0)); // observation
    PolicyGraph::Node *currGraphNode = policy.getRoot(rootIndex);
    PolicyGraph::Node *nextGraphNode;

    MAction* action = &(policy.getAction(currGraphNode));

    // Run simulation
    for (long t=0; t < length; t++){
        for(MState::iterator it=currState.begin(); it != currState.end(); ++it)
            fp << *it << " ";
        fp<<endl;
...
void Simulator::runSingle(long length, double& sumDiscounted,
                          MState startState, PolicyGraph::Node *currNode,
