...
KalmanFilterPlayer::KalmanFilterPlayer(SeekerHSParams* params) : AutoPlayer(params) {
    DEBUG_KF(cout<<"KalmanFilter"<<endl;);
}
...
bool KalmanFilterPlayer::initBeliefRun() {
    ...
    DEBUG_KF(cout<<"Initializing KalmanFilter:"<<endl);
    ...
    DEBUG_KF(cout << "Motion noise (Q): "<<endl<<Q<<endl;);

    //set obs noise
    Eigen::Matrix2d R = Eigen::Matrix2d::Constant(_params->contHiderObsStdDev);
    _kf.setObsNoise(R);
    DEBUG_KF(cout << "Obs noise (R): "<<endl<<R<<endl;);
...
Pos KalmanFilterPlayer::getNextPosRun(int actionDone, int *newAction) {
    if (!playerInfo.hiderObsPosWNoise.isSet()) {
        cout << "KalmanFilterPlayer::getNextPosRun: WARNING: the hider position is not set, only doing prediction."<<endl;
    ...
    //predict
    DEBUG_KF(cout<< "KF predict: "<<flush;);
    _kf.predict();
    DEBUG_KF(cout<< "ok"<<endl;);
    ...
    DEBUG_KF(cout << "State (x): "<<endl<<x<<endl;);
    DEBUG_KF(cout << "State cov. (P): "<<endl<<_kf.getStateCov()<<endl;);
    ...
    DEBUG_KF(cout << "return pos: "<<endl<<returnPos.toString()<<endl;);
