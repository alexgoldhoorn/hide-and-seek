...
FromListHider::FromListHider(SeekerHSParams* params,  std::string listFile) : AutoWalker(params, 1) { //} ), AutoWalker(1) {
...
void FromListHider::writePosFile(AutoWalker* autoWalker, GMap* map, string outFile, uint numHiders, uint numSteps, bool useContinuous) {
    ...
    //init poses
    DEBUG_CLIENT(cout<<"Init with random seeker pos: "<<seekerPlayerInfo.currentPos.toString()<<", and hider pos: "<<hiderPlayerInfo->currentPos.toString()<<endl;);

    //init
    autoWalker->initBelief(map, &seekerPlayerInfo); //map, seekerPos, hiderPos, false);

    //run
    for(uint i=0; i < numSteps; i++) {
        seekerPlayerInfo.currentPos = map->genRandomPos(); //seekerPos = map->genRandomPos();
        hiderPlayerInfo->currentPos = map->genRandomPos(); //hiderPos.set( map->genRandomPos() );
        //next poses of autowalkers
        vector<IDPos> posVec = autoWalker->getAllNextPos(seekerPlayerInfo.currentPos, hiderPlayerInfo->currentPos); //seekerPos,hiderPos);
        bool isFirst = true;
        //write to file
