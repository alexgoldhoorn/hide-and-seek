...
void GMapWidget::drawMap(QPainter& painter) {
    ...
    for (int r=0; r<_gmap->rowCount(); r++) {
        for (int c=0; c<_gmap->colCount(); c++) {
            ...
            if (_player.getMap()!=NULL && playerPos.isSet()) {
    ...
    if (playerPos.isSet()) {
        ...
        if (!_playerInfoVec.empty()) {
            ...
            for(const IDPos& dPos: dynObstVec) { //size_t i=0; i<dynObstVec.size(); i++) {
                bool visib = (_showAllPlayers || _debugMode || _gmap->isVisible(playerPos, dPos, false) );
                if (visib) {
            ...
            for (PlayerInfo* playerInfo : _playerInfoVec) {
                if (*playerInfo!=_player.playerInfo) {
                    //check visibility
                    bool visib = (_showAllPlayers || _gmap->isVisible(playerPos, playerInfo->currentPos, true) );
                    if (visib) {
