...
class PolicyGraph
{
  public:
    ...
    */
    struct Node
    {
        ...
        Node(MAction action): action(action) {}
    };
    ...
    */
    inline MAction& getAction(Node *ptr)
    {
        return ptr->action;
    }

    inline MAction& getAction(long i)
    {
    ...
    */
    inline Node *getNextState(Node *ptr, const MObs& obs)
    {
        return find(obs,ptr->edges);
    }

    inline long getNextNodeIndex(long curr, MObs& obs)
    {
  ...
  private:
    ...
    Node *find(const MObs& obs, const std::vector<Edge>& edges);
