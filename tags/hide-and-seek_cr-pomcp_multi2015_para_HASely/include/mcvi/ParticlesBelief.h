...
class ParticlesBelief : public Belief
{
  public:
    ...
    MState average() const;
    Belief* nextBelief(const MAction& action, const MObs& obs) const;
    ...
    */
    static Belief *beliefFromState(const MState& st, const MObs& obs,  long pathLength);

    /**
       Create a belief from a set of DIFFERENT states
    */
    static Belief *beliefFromStateSet(const std::vector<MState>& st, const MObs& obs, const std::vector<long>& pathLength);
