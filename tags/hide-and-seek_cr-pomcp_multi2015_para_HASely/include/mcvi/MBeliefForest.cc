...
inline map<MObs,ObsEdge>::iterator BeliefForest::findBestObs(Belief *currNode, double target, double& excessUncertainty)
{
    map<MObs,ObsEdge>::iterator iter = currNode->beliefNode->actNodes[currNode->beliefNode->bestUBoundAct.actNum]->obsChildren.begin();
    double currBest = -FLT_MAX;
    map<MObs,ObsEdge>::iterator currBestObs;
