...
void SimulatorTest::hsSimulatorTestDiscSimple() {
    ...
    testSimulator(&sim, map,MAX_STEP_DIST_TEST);
    delete map;
}
...
void SimulatorTest::hsSimulatorContTestContSimple() {
    ...
    testSimulator(&sim, map,MAX_STEP_DIST_TEST_WNOISE);
    delete map;
}
                ...
                for(yRow=0; yRow<map->rowCount(); yRow++) {
                    for(yCol=0; yCol<map->colCount(); yCol++) {
                        ...
                        if (!map->isObstacle(yRow,yCol)) {
                            ...
                            cout << "New isVisible: "<< map->isVisible(xRow,xCol,yRow,yCol,_params->takeDynObstOcclusionIntoAccountWhenLearning)<<endl;

                            cout << "Invisible from pos 1:"<<endl;
                            vector<Pos> invisPosVec = map->getInvisiblePoints(xRow,xCol,_params->takeDynObstOcclusionIntoAccountWhenLearning);
                            ...
                            bool v = map->isVisible(xRow,xCol,yRow,yCol,_params->takeDynObstOcclusionIntoAccountWhenLearning);
                            ...
                            for (const State* state : allInitStates) { //  vector<State*>::iterator it = allInitStates.begin(); it != allInitStates.end(); it++) {
                                QVERIFY(state!=NULL);
