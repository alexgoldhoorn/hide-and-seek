...
FastInfUBInitializer::FastInfUBInitializer( SharedPointer<MOMDP> problem, BeliefValuePairPoolSet* _bound, bool _onlyMDPInit) { //ag120420
    pomdp = ( SharedPointer<MOMDP>) problem;
    bound = _bound;
    onlyMDPInit = _onlyMDPInit; //ag120420: bool to set if only using MDP init or als FIB init (default)
...
FastInfUBInitializer::FastInfUBInitializer(SharedPointer<MOMDP> problem) {
    pomdp = ( SharedPointer<MOMDP>) problem;
    onlyMDPInit = false; //ag120420: bool to set if only using MDP init or als FIB init (default)
...
void FastInfUBInitializer::initFIB(double targetPrecision, bool getFIBvectors) {
    ...
    //ag: set alpha to be the mdp upper bound
    initMDP(MDP_RESIDUAL, M); // SYL260809 modified

    //cout <<" initMDP done, "<<((int)(time(NULL)-startT)) << "s"<<endl;
    ...
    */
    ...
    //cout <<" done for A, "<<((int)(time(NULL)-startT)) << "s"<<endl;
    ...
    //startT = time(NULL);
//#ifdef ag_undef
    if (onlyMDPInit) { //ag120420
        DEBUG_LOG(cout << "No FIB initialization, only MDP."<<endl;);
    } else {
        DEBUG_LOG(cout << "Upper bound FIB init starting..."<<endl;);
        do {
            DEBUG_LOG(cout << "iteration: maxResidual="<<maxResidual<<",targetPrec="<<targetPrecision<<endl;);
            ...
            FOR (a, pomdp->actions->size()) {
                ...
                FOR (Xc, pomdp->XStates->size()) {
                    ...
                    FOR (o, pomdp->observations->size()) {
                        ...
                        FOR (Xn, pomdp->XStates->size()) {
                            if (!(pomdp->XTrans->getMatrix(a, Xc)->isColumnEmpty(Xn))) {
...
//#endif
    ...
    //cout <<" end loop, "<<((int)(time(NULL)-startT)) << "s"<<endl;
    ...
    */
    ...
    if (!getFIBvectors) { //ag: default (getFIBvectors=false) -> set to bounds
