...
namespace momdp
{
        //ag WARNING: be sure to have added correctly the items: forall col: forall row: add
        //ag NOTE: here 0.0 is returned in cases where no value has been defined (therefore 'SparseMatrix')
			...
			if (di->index >= r) {
				if (di->index == r) {
					return di->value;
                                    ...
                                    //cout << " ("<<r<<","<<c<<") di.index:"<<di->index<<endl;
                                        return 0.0;
				}
			}
		}
