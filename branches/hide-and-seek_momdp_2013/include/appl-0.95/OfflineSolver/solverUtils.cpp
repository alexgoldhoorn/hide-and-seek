...
namespace momdp{
	...
	**********************************************************************/
    ...
    //AG120420: version in string
    string SolverParams::getAPPLVersion() {
        stringstream ss;
        ss<<"Approximate POMDP Planning (APPL) Toolkit Version 0.9x [AG edit] [build "<<__DATE__<<" "<<__TIME__<<"]";
        return ss.str();
    }

    //ag120420: get if debug log on
    bool SolverParams::isDebugOn() {
#ifdef DEBUG_LOG_ON
        return true;
#else
        return false;
    ...
    //ag120420: get if debug trace on (very detailed debug)
    bool SolverParams::isDebugTraceOn() {
	...
	SolverParams::SolverParams(void)
	{
		...
		QMDPSolution = false;
		FIBSolution = false;
        FIB_UB_Init = true; //ag120426: FIB upper bound init
		useLookahead = true;
        ...
        //ag120423: init values
        cb_initialization_precision_factor = CB_INITIALIZATION_PRECISION_FACTOR;
        //AG121008: do pruning or not
        doPruning = true;
        //AG121016: show params
        showParams = false;
        //AG121212: max depth stop
        maxTreeDepth = -1;
	}
    ...
    //AG121016: option to show all params
        { "show-params", 0, NULL, '1' },

    //AG121212: max depth of tree
        { "max-depth", 1, NULL, 'x' },
		...
		while (1) 
		{
			...
			case 'h': // help
				...
				break;
			case 'V': // version
                //ag120420: get version with compile date/time
                cout << getAPPLVersion() << endl;
                cout << "Debug Log: "<< (isDebugOn()?"on":"off") << "; debug trace: "<< (isDebugTraceOn()?"on":"off")<<endl;
				exit(EXIT_SUCCESS);
				break;
			case 's': // search
			...
			case 'R':
				...
				break;
            case '1': ////AG121016: option to show all params
                p.showParams = true;
                break;
            case 'x':
                p.maxTreeDepth = atoi(optarg);
                break;
			case '?': // unknown option
    ...
    void SolverParams::showAllParams() {
        cout << "*** APPL solver params ***" <<endl
             << "---------------------------------------" << endl;

        cout<< "\tmemoryLimit = " <<memoryLimit << endl;
        cout<< "\tuseLookahead = " <<useLookahead << endl;
        cout<< "\tpolicyFile = " <<policyFile << endl;
        cout<< "\toutputFile = " <<outputFile << endl;
        cout<< "\tproblemName = " <<problemName << endl;
        cout<< "\tstateMapFile = " <<stateMapFile << endl;
        cout<< "\tproblemBasenameWithoutPath = " <<problemBasenameWithoutPath << endl;
        cout<< "\tproblemBasenameWithPath = " <<problemBasenameWithPath << endl;
        cout<< "\tpolicyGraphFile = " <<policyGraphFile << endl;
        cout<< "\thardcodedProblem = " <<hardcodedProblem << endl;

        cout<< "\tgraphDepth = " <<graphDepth << endl;
        cout<< "\tgraphMaxBranch = " <<graphMaxBranch << endl;
        cout<< "\tgraphProbThreshold = " <<graphProbThreshold << endl;

        cout<< "\tMDPSolution = " <<MDPSolution << endl;
        cout<< "\tQMDPSolution = " <<QMDPSolution << endl;
        cout<< "\tFIBSolution = " <<FIBSolution << endl;

        cout<< "\tFIB_UB_Init = " <<FIB_UB_Init << endl;

        cout<< "\tsimLen = " <<simLen << endl;
        cout<< "\tsimNum = " <<simNum << endl;
        cout<< "\tseed = " <<seed << endl;

        cout<< "\tcmdName = " <<cmdName << endl;
        cout<< "\tstrategy = " <<strategy << endl;
        cout<< "\ttargetTrials = " <<targetTrials << endl;

        cout<< "\ttargetPrecision = " <<targetPrecision << endl;
        cout<< "\tuseFastParser = " <<useFastParser << endl;
        cout<< "\tdoConvertPOMDP = " <<doConvertPOMDP << endl;
        cout<< "\toutPolicyFileName = " <<outPolicyFileName << endl;
        cout<< "\tinterval = " <<interval << endl;
        cout<< "\ttimeoutSeconds = " <<timeoutSeconds << endl;
        cout<< "\tdelta = " <<delta << endl;
        cout<< "\toverPruneThreshold = " <<overPruneThreshold << endl;
        cout<< "\tlowerPruneThreshold = " <<lowerPruneThreshold << endl;
        cout<< "\tdynamicDeltaPercentageMode = " <<dynamicDeltaPercentageMode << endl;
        cout<< "\tBP_IMPROVEMENT_CONSTANT = " <<BP_IMPROVEMENT_CONSTANT << endl;

        cout<< "\tdumpData = " <<dumpData << endl;
        cout<< "\tdumpPolicyTrace = " <<dumpPolicyTrace << endl;
        cout<< "\tdumpPolicyTraceTime = " <<dumpPolicyTraceTime << endl;
        cout<< "\trandomizationBP = " <<randomizationBP << endl;


        cout<< "\tcb_initialization_precision_factor = " <<cb_initialization_precision_factor << endl;
        cout<< "\tdoPruning = " <<doPruning << endl;
        cout<< "\tmaxTreeDepth = " <<maxTreeDepth<<endl;
        cout<< "\tshowParams = " <<showParams << endl;
        cout << "---------------------------------------" << endl;
    }
