...
namespace momdp
{
	...
	{
            /*cout << "samplebp.sample"<<endl;
            cout << " row: " << currIndexRow.row << "; sval: "<<currIndexRow.sval<<"; currRoot: "<<currentRoot<<endl;*/
		isRoot = false;
        ...
        //AG130116: added bool stopTrial
        bool stopTrial = false;

        if ( (finalExcess) <= 0 ) { //if final precision reached
            stopTrial = true;
            DEBUG_TRACE(cout<<"Stopping trial because of finalExcess"<<endl;);
            cout << " p"<<numTrials<<" ";

        } else if ( solver->solverParams->maxTreeDepth > 0  &&  depthArr[currentRoot] > solver->solverParams->maxTreeDepth ) {
            //AG130116: add limit on search depth (if param set)

            stopTrial = true;
            DEBUG_TRACE(cout<<"Stopping trial because reached max tree depth."<<endl;);
            cout << " d"<<numTrials<<" ";
        }
...
// doesnt seem to be used? - SYL 23072008
    ...
    //AG121211: get depth of root
    int SampleBP::getDepth(unsigned int currentRoot) {
        return depthArr[currentRoot];
    }
