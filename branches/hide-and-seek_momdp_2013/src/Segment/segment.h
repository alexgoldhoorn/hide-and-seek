...
public:
     ...
     */
    virtual vector<int>* segment(vector<double>* vec, int &segmentCount)=0;

    //! segments current _vecSeg using conneciton analysis
    vector<int>* connectionAnalysis(vector<int>* vec, bool ignoreObstacles=false);
    ...
    */
    vector<int>* get0BasedSegments(vector<int>* vec, int &segmentCount);

    //! Show the categorized map
    void showMap(vector<int>* segVec, int xState=-1, int yState=-1);
...
protected:
    ...
    vector<int>* newRegVector(int size, bool init0);
...
private:
    //! check neighbour
    void caChangeItem(vector<int>* newRegVec, vector<int>* vecSeg, int r, int c, int r1, int c1, int i, bool ignoreObstacles);
};
