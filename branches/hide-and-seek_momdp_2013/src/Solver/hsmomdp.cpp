...
bool HSMOMDP::initBase(const char* pomdpFile, const char* logOutFile, const char *timeLog) {
    ...
    argv_appl[0] = "pomdpsol";
    argv_appl[1] = charConstToCharArr(pomdpFile);

    if (logOutFile != NULL) {
        //open log file
        argv_appl[2] = "--output-file";
...
bool HSMOMDP::initBelief(GMap* gmap, Pos seekerInitPos, Pos hiderInitPos, bool hiderVisible) {
    ...
    int numStates = _gmap->numFreeCells();
    ...
    _gmap->printMap(seekerInitPos.row,seekerInitPos.col); //ag130301: other syntax printmap
    DEBUG_HS_INIT(cout << " seeker init pos: "<<seekerInitPos.row<<","<<seekerInitPos.col<<endl;);

    int seekerInitPosI = _gmap->getIndexFromCoord(seekerInitPos); //ag130304: was getPosIndex

    DEBUG_HS_INIT(cout <<"the initial position of the seeker is ("<<seekerInitPos.row<<", "<<seekerInitPos.col<<") index num: "<<seekerInitPosI<<endl;);
    ...
    if (hiderVisible) {
        ...
        DEBUG_HS_INIT(cout<<"hider is visible with pos ("<<hiderInitPos.row<<", "<<hiderInitPos.col<<") index num: "<<hiderInitPosI<<endl;);
        ...
        vector<Pos> invisPosVector = _gmap->getInvisiblePoints(seekerInitPos.row,seekerInitPos.col);
        ...
        for (vector<Pos>::iterator it = invisPosVector.begin() ; it != invisPosVector.end(); ++it) {
            int i = _gmap->getIndexFromCoord(it->row,it->col);
    ...
    _unobservedObs = _gmap->numFreeCells();
    ...
    //show belief
    DEBUG_PB(HSSolver::printBelief(_currBelief,"initial belief",hiderInitPosI,_gmap););
...
bool HSMOMDP::updateBelief(int observ, int x_state) {
    ...
    //show belief
    DEBUG_PB(HSSolver::printBelief(_currBelief,"update belief",observ,_gmap););
...
bool HSMOMDP::updateBelief(Pos seekerPos, Pos hiderPos, bool visible) {
    int xI = _gmap->getIndexFromCoord(seekerPos);

    int obsI = _unobservedObs;
    if (visible) {
        obsI = _gmap->getIndexFromCoord(hiderPos);
    }
...
int HSMOMDP::getNextAction(Pos seekerPos, Pos hiderPos, bool visible) {
    int xI = _gmap->getIndexFromCoord(seekerPos);
...
double HSMOMDP::getBelief(int r, int c) {
    return getBelief(_gmap->getIndexFromCoord(r,c));
}
