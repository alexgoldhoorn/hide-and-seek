...
RandomListHider::RandomListHider(string actionFileName) : RandomHider()
{
...
int RandomListHider::getNextAction(Pos seekerPos, Pos hiderPos, bool opponentVisible) {
...
void RandomListHider::readActionFile(string actionFileName) {
    ...
    if (aFile.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        //convert init pos
        bool ok = false;
        _initActionListPos.row = /*_curpos.x =*/ posList[0].toInt(&ok);
        if (ok) _initActionListPos.col =/* _curpos.y =*/ posList[1].toInt(&ok);
        if (!ok) {
            cout << "ERROR: Action file, 2nd line should be init pos, could not parse the numbers (eg: 1,3): "<<actionFileName<<endl;
            exit(-1);
        }

        DEBUG_HS(cout << "Initial position: r"<<_initActionListPos.row<<"c"<<_initActionListPos.col<<endl;);
...
int RandomListHider::getNextActionFromList() {
    int a = -2;
    if (_recActionList.size()==0 || _recordActionI<0 || _recordActionI>=_recActionList.size()) a=-1;

    a = _recActionList[_recordActionI];
    _recordActionI++;
    if (_recordActionI>=_recActionList.size()) cout << "Warning: end of aciton list reached"<<endl;
...
string RandomListHider::getActionListName() {
