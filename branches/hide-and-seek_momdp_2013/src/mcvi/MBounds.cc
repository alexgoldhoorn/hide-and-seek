...
void Bounds::backUp(Belief& belief)
{
    ...
    for (map<MObs,ObsEdge>::iterator iter = beliefNode.actNodes[actIndex]->obsChildren.begin();
...
void Bounds::backUpInitPolicies(Belief& belief)
{
    ...
    for (long i = 0; i < model.getNumInitPolicies(); i++){
        if (beliefNode.actNodes[i] == NULL)
            beliefNode.actNodes[i] = new ActNode(*(new MAction(i)),
                                                 belief,
                                                 this);
        ...
        for (long j = 0; j < numRandStreams; j++){
            ...
            MState nextState(model.getNumStateVar(),0);

            long currMacroActState = InitMacroActState;
            long nextMacroActState;

            MObs obs(vector<long>(model.getNumObsVar(),0));
            ...
            for (long k = 0; k < maxSimulLength; k++){
                ...
                currReward = model.initPolicy(*currState, MAction(i), currMacroActState, nextState, nextMacroActState, obs, randStream);
        ...
        MObs tempObs(vector<long>(model.getNumObsVar(),0));
        model.setObsType(tempObs,LoopObs);
        pair<map<MObs,ObsEdge>::iterator, bool> ret =
                beliefNode.actNodes[i]->obsChildren.insert(pair<MObs,ObsEdge >(tempObs,ObsEdge(tempObs, this)));
...
void Bounds::backUpActions(Belief& belief)
{
    ...
    for (long i = model.getNumInitPolicies();
         i < model.getNumInitPolicies() + model.getNumActs();
         i++){
        if (model.allowableAct(belief, MAction(i))){
            if (beliefNode.actNodes[i] == NULL)
                beliefNode.actNodes[i] = new ActNode(*(new MAction(i)),
                                                     belief,
                                                     this);
