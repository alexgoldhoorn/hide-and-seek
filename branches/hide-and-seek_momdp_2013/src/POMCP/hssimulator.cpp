...
void HSSimulator::getActions(State* state, History* history, std::vector<int>& actions, bool smart) {
    assert(state!=NULL);
...
void HSSimulator::getPossibleNextPos(Pos &prevPos, vector<Pos>& posVec, int obs) {
    Pos obsPos;
    if (obs==_hiddenObs) {
        Pos p = _map->getCoordFromIndex(obs);
        obsPos.set(p);
    }
    ...
    FORs(a,HSGlobalData::NUM_ACTIONS) {
        ...
        if (newPos.isSet() && (obs==-1 || obsPos.equals(newPos))) { //todo: should i put a check of consistency
            posVec.push_back(newPos);
        }
    }
}

void HSSimulator::setInitialNodeValue(State* state, History* history, BaseNode* node, int action) {
    double r = 0;
    if (action==-1) {
...
State* HSSimulator::step(State *state, int action, int &obsOut, double &reward, int obsIn) {
    assert(state!=NULL);
    ...
    getPossibleNextPos(hsstate->hiderPos, posVec, obsIn);

    if (obsIn>=0 && posVec.size()==0) {
        DEBUG_POMCP(cout << "HSSimulate::step: no consistent ";);
        getPossibleNextPos(hsstate->hiderPos, posVec);
    }
...
char HSSimulator::getGameStatus(HSState *state) {
    if (seekerWin(state)) {
...
int HSSimulator::getObservation(Pos seekerPos, Pos hiderPos, bool opponentVisible) {    
    //assert(!opponentVisible && (!hiderPos.isSet() || _map->isVisible(seekerPos,hiderPos));
    assert(hiderPos.isSet() && opponentVisible || !opponentVisible);
    if (!opponentVisible) {
        return _hiddenObs;
    } else {
        return _map->getIndexFromCoord(hiderPos);
    }
}
